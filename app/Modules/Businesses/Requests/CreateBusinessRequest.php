...
class CreateBusinessRequest extends FormRequest
{
    ...
    {
        $lang = $this->input('language_preference', $this->header('Accept-Language', 'fr'));
        if ($lang === 'en') {
            return [
                'industry_id.required' => 'Industry is required.',
                'name_fr.required'     => 'Business name in French is required.',
            ];
        }
        return [
            'industry_id.required' => 'Le secteur d\'activité est obligatoire.',
            'name_fr.required'     => 'Le nom de l\'entreprise en français est obligatoire.',
        ];
    }
}
