...
<div class="max-w-[1536px] mx-auto flex items-stretch">
    ...
    <main class="flex-1 min-w-0 px-4 lg:px-6 py-6">
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h1 class="text-[22px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? 'Créer une proposition de devis' : 'Create a quote proposal' }}</h1>
                <p class="mt-1 text-[13px] text-[#55524A] dark:text-[#B4B5A6]">{{ $isFr ? 'Construisez et personnalisez votre proposition pour l\'envoyer à l\'artisan.' : 'Build and customise your proposal to send it to the artisan.' }}</p>
            </div>
        ...
        <div class="mt-5 ui-card overflow-x-auto">
            <div class="flex items-center gap-3 min-w-[860px]">
                ...
                <div class="relative flex items-center gap-3.5 shrink-0 {{ $stIdx === 1 ? 'pb-1' : '' }}">
                    <span class="w-[36px] h-[36px] shrink-0 rounded-full flex items-center justify-center text-[14px] font-bold {{ $stIdx === 1 ? 'bg-[#0E5A2D] dark:bg-[#2E9250] text-white dark:text-[#04150A]' : 'bg-white dark:bg-[#12150F] border border-[#D9DDD9] dark:border-[#39402F] text-[#55524A] dark:text-[#B4B5A6] ' }}">{{ $stNum }}</span>
                    <span>
                        <span class="block text-[13px] font-bold {{ $stIdx === 1 ? 'text-[#14652F] dark:text-[#339B56] ' : 'text-[#1B1B18] dark:text-[#F3EFE7] ' }}">{{ $stTitle }}</span>
                        <span class="block mt-0.5 text-[11.5px] text-[#6F6B60] dark:text-[#868778]">{{ $stSub }}</span>
                    </span>
                    @if($stIdx === 1)<span class="absolute left-0 right-0 -bottom-3 h-[3px] rounded-full bg-[#14652F] dark:bg-[#2E9250]"></span>@endif
                </div>
                @if($stIdx < count($steps) - 1)
                <span class="flex-1 min-w-[36px] h-px bg-[#E0E4E0] dark:bg-[#1A1E16]"></span>
                @endif
                @endforeach
            </div>
        </div>
        ...
        <div class="mt-5 flex flex-col xl:flex-row gap-5 items-start">
                ...
                <section class="ui-card">
                    ...
                    <div class="mt-4 overflow-x-auto">
                        <div class="min-w-[920px]">
                            <div class="grid grid-cols-[minmax(220px,1.4fr)_90px_110px_120px_110px_130px_120px_80px] gap-3 bg-[#F7F8F7] dark:bg-[#12150F] rounded-lg px-3 py-2.5">
                                ...
                                <span class="text-[10.5px] font-bold tracking-[0.04em] text-[#6F6B60] dark:text-[#868778] uppercase {{ $hIdx > 0 && $hIdx < 7 ? 'text-center' : '' }} self-center leading-tight">{{ $h }}</span>
                                @endforeach
                            </div>
                            <div id="bld-rows">
                                @foreach($rows as $rIdx => [$rImg, $rName, $rRef, $rSpec, $rQty, $rPrice, $rDisc, $rTotal])
                                <div class="bld-row border-b border-[#F1F2F1] dark:border-[#262B21] py-3.5">
                                    <div class="grid grid-cols-[minmax(220px,1.4fr)_90px_110px_120px_110px_130px_120px_80px] gap-3 items-center">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="shrink-0 text-[12.5px] text-[#55524A] dark:text-[#B4B5A6]">%</span>
                                        </div>
                                        {{-- TVA is applied once on the proposal total, not per line, so
                                             this reports the rate rather than pretending to be a choice. --}}
                                        <p class="text-[12.5px] text-[#55524A] dark:text-[#B4B5A6] text-center">TVA 19.25%</p>
                                        <p class="bld-total text-[13px] font-bold text-[#14652F] dark:text-[#339B56] text-right whitespace-nowrap">{{ $rTotal }}</p>
                    ...
                    <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
                        <p id="bld-count" class="text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]"><span class="font-bold">0 articles</span> &nbsp;•&nbsp; {{ $isFr ? 'Quantité totale' : 'Total quantity' }}: <span class="font-bold">0 {{ $isFr ? 'unités' : 'units' }}</span></p>
                        <p class="text-[12.5px] text-[#55524A] dark:text-[#B4B5A6]">{{ $isFr ? 'Sous-total' : 'Subtotal' }} &nbsp; <span id="bld-subtotal" class="text-[15.5px] font-bold text-[#14652F] dark:text-[#339B56]">0 FCFA</span></p>
                    </div>
                </section>
                ...
                <div class="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
                    ...
                    <section class="ui-card">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <span class="pb-2.5 text-[13px] text-[#55524A] dark:text-[#B4B5A6]">%</span>
                    ...
                    <section class="ui-card">
                        ...
                        <div class="flex items-end gap-3">
                            <div class="relative flex-1 min-w-0">
                                ...
                                <i data-lucide="calendar" class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#55524A] dark:text-[#B4B5A6] pointer-events-none"></i>
                            </div>
                            <div class="w-[110px]">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[12px] text-[#55524A] dark:text-[#B4B5A6]">{{ $isFr ? 'jours' : 'days' }}</span>
                                </div>
                            </div>
                        </div>
            ...
            <aside class="w-full xl:w-[290px] shrink-0 space-y-4">
                <section class="ui-card">
                    ...
                    @if($isReal)
                    <div class="mt-3.5 border border-[#EDEEED] dark:border-[#262B21] rounded-xl px-4 py-3 grid grid-cols-2 divide-x divide-[#EDEEED] dark:divide-[#262B21]">
                        <div class="pr-3">
                            <p class="ui-dt">{{ $isFr ? 'Référence de la demande' : 'Request reference' }}</p>
                            <a href="{{ route('quotes.create', ['lang' => $lang]) }}" class="ui-dd inline-block text-[#14652F] dark:text-[#339B56] underline underline-offset-2">{{ $builderRfq->reference }}</a>
                        </div>
                        <div class="pl-3">
                    ...
                    <dl class="mt-4 space-y-3">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $fnLabel }}</dt>
                    ...
                    <div class="mt-4 border-t border-[#F0F1F0] dark:border-[#262B21] pt-4 flex items-center justify-between gap-3">
                        <span class="text-[13.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] uppercase">{{ $isFr ? 'Total général' : 'Grand total' }}</span>
                        <span id="fin-grand" class="text-[14.5px] font-bold text-[#14652F] dark:text-[#339B56]">0 FCFA</span>
                    </div>
                </section>
                ...
                <section class="ui-card">
                    ...
                    <ul class="mt-3 space-y-2.5">
                        ...
                        <li class="flex items-center gap-2.5 text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">
                            <i data-lucide="circle-check" class="w-4 h-4 shrink-0 text-[#1F8A4C] dark:text-[#339B56]" style="stroke-width:2"></i>
                            {{ $inc }}
                        </li>
                        @endforeach
                    </ul>
                ...
                <section class="bg-[#FDF8EC] dark:bg-[#12150F] border border-[#F2E8CE] dark:border-[#4A3A12] rounded-2xl px-5 py-4">
                    <h2 class="text-[12.5px] font-bold text-[#C97A16] dark:text-[#EDB33A]">{{ $isFr ? 'Note interne' : 'Internal note' }} <span class="font-semibold">({{ $isFr ? 'non visible par l\'acheteur' : 'not visible to the buyer' }})</span></h2>
                ...
                <section class="bg-[#EDF3FC] dark:bg-[#101C33] rounded-2xl px-5 py-4">
                    <h2 class="text-[12.5px] font-bold text-[#3565DE] dark:text-[#8FB6F5]">{{ $isFr ? 'Informations importantes' : 'Important information' }}</h2>
                    <p class="mt-2 text-[11.5px] text-[#3B382F] dark:text-[#B4B5A6] leading-relaxed">{{ $isFr ? "Cette proposition est confidentielle et destinée uniquement à l'acheteur. Les prix peuvent varier selon les conditions du marché." : 'This proposal is confidential and intended only for the buyer. Prices may vary according to market conditions.' }}</p>
                </section>
            </aside>
        </div>
    </main>
