...
<div class="max-w-[1536px] mx-auto flex items-stretch">
    ...
    <main class="flex-1 min-w-0 px-4 lg:px-7 py-6">
        ...
        <nav class="flex items-center gap-2 text-[12.5px]">
            <a href="{{ route('quotes.index', ['lang' => $lang]) }}" class="font-semibold text-[#157A43] dark:text-[#339B56] hover:text-[#14532D] dark:hover:text-[#339B56]">{{ $isFr ? 'Mes demandes & devis' : 'My requests & quotes' }}</a>
            <i data-lucide="chevron-right" class="w-3.5 h-3.5 text-[#55524A] dark:text-[#B4B5A6]"></i>
            <span class="font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? 'Détail de la proposition' : 'Proposal detail' }}</span>
        </nav>
        <div class="mt-2 flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1.5 text-[13px] text-[#55524A] dark:text-[#B4B5A6]">
                    {{ $isFr ? 'Proposée par' : 'Proposed by' }} <span class="font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $realBizName }}</span>
                    &nbsp;•&nbsp; {{ $isFr ? 'Envoyée le' : 'Sent on' }} <span class="font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $realSentAt }}</span>
                    &nbsp;•&nbsp; <span class="font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $realRef }}</span>
                </p>
            </div>
        ...
        <div class="mt-5 flex flex-col 2xl:flex-row gap-5 items-start">
            <div class="flex-1 min-w-0 w-full">
                ...
                <section class="ui-card flex flex-col lg:flex-row gap-6 lg:divide-x divide-[#F0F1F0] dark:divide-[#262B21]">
                    <div class="flex items-start gap-4 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            @endif
                            <a href="{{ $vendorUrl }}" class="mt-2 inline-block text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7] underline underline-offset-4 hover:text-[#14652F] dark:hover:text-[#339B56]">{{ $isFr ? 'Voir le profil de l\'artisan' : 'View the artisan\'s profile' }}</a>
                        </div>
                    </div>
                    <div class="lg:px-6">
                    ...
                    <div class="lg:pl-6 flex items-start gap-3.5">
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-[11.5px] text-[#6F6B60] dark:text-[#868778]">({{ max($realDaysLeft, 0) }} {{ $isFr ? 'jours restants' : 'days left' }})</p>
                            @endif
                        </div>
                    </div>
                </section>
                ...
                <section class="mt-4 ui-card ui-card--flush">
                    <div class="px-5 pt-1 border-b border-[#F0F1F0] dark:border-[#262B21] overflow-x-auto">
                        <div class="flex items-center gap-7 min-w-max">
                            ...
                            @if($tKey === 'details')
                            <span class="relative py-4 text-[13px] font-bold text-[#14652F] dark:text-[#339B56] whitespace-nowrap">
                                {{ $tLabel }}
                                <span class="absolute left-0 right-0 -bottom-px h-[3px] rounded-t bg-[#14652F] dark:bg-[#2E9250]"></span>
                            </span>
                            @else
                            <a href="{{ $tUrl }}" class="py-4 text-[13px] text-[#55524A] dark:text-[#B4B5A6] hover:text-[#1B1B18] dark:hover:text-[#F3EFE7] whitespace-nowrap inline-flex items-center gap-2">
                                {{ $tLabel }}
                                @if($tBadge)<span class="bg-[#EFF0EF] dark:bg-[#1A1E16] rounded-full px-2 py-0.5 text-[10.5px] font-bold text-[#3B382F] dark:text-[#B4B5A6]">{{ $tBadge }}</span>@endif
                            </a>
                            @endif
                            @endforeach
                        </div>
                    ...
                    <div class="px-5 py-5">
                        <h2 class="ui-card-title">{{ $isFr ? 'Articles proposés' : 'Proposed items' }} <span class="font-normal text-[#55524A] dark:text-[#B4B5A6]">({{ count($rows) }} {{ $isFr ? 'articles' : 'items' }})</span></h2>
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-top">
                                            <div class="flex items-start gap-3">
                                                ...
                                                <p class="text-[12.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] max-w-[150px] leading-snug">{{ $rName }}</p>
                                            </div>
                                        </td>
                                        <td class="align-top text-[#3B382F] dark:text-[#B4B5A6] leading-relaxed max-w-[190px]">{{ $rDesc }}</td>
                                        <td class="align-top text-center font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $rQty }}</td>
                                        <td class="align-top text-center text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr ? 'Pièces' : 'Pieces' }}</td>
                                        <td class="align-top text-right font-semibold text-[#1B1B18] dark:text-[#F3EFE7] whitespace-nowrap">{{ $rPrice }}</td>
                                        <td class="align-top text-center text-[#3B382F] dark:text-[#B4B5A6]">{{ $rDisc }}</td>
                                        <td class="align-top text-right text-[#3B382F] dark:text-[#B4B5A6] whitespace-nowrap">{{ $rTax }}</td>
                                        <td class="align-top text-right font-bold text-[#14652F] dark:text-[#339B56] whitespace-nowrap">{{ $rTotal }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="mt-5 border-t border-[#F0F1F0] dark:border-[#262B21] pt-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div>
                                <h3 class="text-[13px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? 'Conditions de paiement' : 'Payment terms' }}</h3>
                                <p class="mt-1.5 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $rp->payment_terms ?: '—' }}</p>
                                <h3 class="mt-4 text-[13px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">Incoterms</h3>
                                <p class="mt-1.5 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $rp->incoterms ?: '—' }}</p>
                            </div>
                            <div>
                                <h3 class="text-[13px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? 'Délais de production' : 'Production time' }}</h3>
                                <p class="mt-1.5 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $rp->production_delay ?: '—' }}</p>
                                <h3 class="mt-4 text-[13px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? 'Délais de livraison' : 'Delivery time' }}</h3>
                                <p class="mt-1.5 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $rp->delivery_delay ?: '—' }}</p>
                            </div>
                            <div>
                                <dl class="space-y-2.5">
                                    @foreach($totals as [$ttLabel, $ttValue, $ttColor])
                                    <div class="flex items-center justify-between gap-3">
                                        <dt class="text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $ttLabel }}</dt>
                                ...
                                <div class="mt-4 border-t border-[#F0F1F0] dark:border-[#262B21] pt-4 flex items-center justify-between gap-3">
                                    <span class="text-[14.5px] font-bold text-[#157A43] dark:text-[#339B56] uppercase">{{ $isFr ? 'Total général' : 'Grand total' }}</span>
                                    <span class="text-[15.5px] font-bold text-[#157A43] dark:text-[#339B56]">{{ number_format($rp->total) }} FCFA</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-4 bg-[#EDF3FC] dark:bg-[#101C33] rounded-xl px-5 py-3.5 flex items-center gap-3">
                    <i data-lucide="info" class="w-4 h-4 shrink-0 text-[#3565DE] dark:text-[#8FB6F5]"></i>
                    <p class="text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr ? 'Cette proposition est confidentielle et destinée uniquement à l\'acheteur.' : 'This proposal is confidential and intended only for the buyer.' }}</p>
                </div>
            </div>
            ...
            <aside class="w-full 2xl:w-[300px] shrink-0 space-y-4">
                <section class="ui-card">
                    ...
                    <p class="mt-3.5 flex items-center gap-2.5">
                        <span class="w-[22px] h-[22px] shrink-0 rounded-full bg-[#F5B301] dark:bg-[#3A2B06] flex items-center justify-center"><i data-lucide="clock" class="w-3 h-3 text-white" style="stroke-width:2.6"></i></span>
                        <span class="ui-pill ui-pill-warn">{{ $realStatusFr }}</span>
                    </p>
                    <p class="mt-3 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6] leading-relaxed">{{ $isFr ? 'L\'artisan attend votre réponse.' : 'The artisan awaits your reply.' }}<br>{{ $isFr ? 'Délai estimé : 1 à 3 jours ouvrables' : 'Estimated time: 1 to 3 working days' }}</p>
                </section>
                ...
                <section class="bg-[#EFF6F1] dark:bg-[#1A1E16] rounded-2xl px-5 py-4 flex items-start gap-3">
                    <i data-lucide="piggy-bank" class="w-[22px] h-[22px] shrink-0 text-[#1F8A4C] dark:text-[#339B56]" style="stroke-width:1.7"></i>
                    <p class="text-[12px] leading-relaxed">
                        <span class="font-bold text-[#157A43] dark:text-[#339B56]">{{ $isFr ? 'Vous économisez' : 'You save' }}</span><br>
                        <span class="text-[#3B382F] dark:text-[#B4B5A6]">{{ number_format((int) $rp->discount_amount) }} FCFA {{ $isFr ? 'sur cette proposition' : 'on this proposal' }}</span>
                    </p>
                </section>
                @endif

                <section class="ui-card">
                    <h2 class="ui-card-title text-[#3565DE] dark:text-[#8FB6F5]">{{ $isFr ? 'Informations importantes' : 'Important information' }}</h2>
                    <ul class="mt-3.5 space-y-3">
                        @foreach($importantInfo as [$iiIcon, $iiLabel])
                        <li class="flex items-center gap-3 text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">
                            <i data-lucide="{{ $iiIcon }}" class="w-[17px] h-[17px] shrink-0 text-[#55524A] dark:text-[#B4B5A6]" style="stroke-width:1.6"></i>
                            {{ $iiLabel }}
                        </li>
                        @endforeach
                    </ul>
