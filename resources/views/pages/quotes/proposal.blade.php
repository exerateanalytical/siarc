@php
    ...
    $propInfo = [
        [$isFr ? 'Référence de la demande' : 'Request reference', 'RFQ-2024-000189'],
        [$isFr ? 'Artisan / Entreprise' : 'Artisan / Business',   'Art Bois Nature'],
        [$isFr ? 'Date de réponse' : 'Response date',             $isFr ? '25 Mai 2024' : '25 May 2024'],
        [$isFr ? 'Validité de l\'offre' : 'Offer validity',       '25/06/2024 (30 ' . ($isFr ? 'jours' : 'days') . ')'],
        [$isFr ? 'Devise' : 'Currency',                           'FCFA - Franc CFA'],
        ['Incoterms',                                             'FOB - Free On Board'],
        [$isFr ? 'Lieu de livraison' : 'Delivery location',       'Marseille, France'],
        [$isFr ? 'Délai de production' : 'Production time',       $isFr ? '7 - 10 jours ouvrables' : '7 - 10 working days'],
        [$isFr ? 'Délai de livraison' : 'Delivery time',          $isFr ? '15 - 20 jours ouvrables' : '15 - 20 working days'],
    ];
    ...
    $propDocs = [
        ...
        ['qb-file-2.png', 'Logo-entreprise.png',     '1.2 MB'],
    ];

    // [thumb, name, ref, desc, qty, unit, price, discount, tax, total]
    $docRows = [
    ...
    $docNotes = $isFr ? [
        'Les délais sont donnés à titre indicatif et peuvent varier selon le planning de production.',
        ...
        'Lead times are indicative and may vary depending on the production schedule.',
    ...
    // [icon, label, value]
    $docChips = [
        ['file-text',     'Incoterms',                                     'FOB - Free On Board'],
        ['circle-dollar-sign', $isFr ? 'Devise' : 'Currency',              'FCFA - Franc CFA'],
        ['calendar-days', $isFr ? 'Validité de l\'offre' : 'Offer validity', '30 ' . ($isFr ? 'jours' : 'days') . ' (25/06/2024)'],
        ['clock',         $isFr ? 'Délai de production' : 'Production time', $isFr ? '7 - 10 jours ouvrables' : '7 - 10 working days'],
        ['truck',         $isFr ? 'Délai de livraison' : 'Delivery time',  $isFr ? '15 - 20 jours ouvrables' : '15 - 20 working days'],
    ];
    ...
    <main class="flex-1 min-w-0 px-4 lg:px-6 py-6">
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-[13px] text-[#55524A]">{{ $isFr ? 'Vérifiez les détails de votre proposition avant de l\'envoyer à l\'artisan.' : 'Check the details of your proposal before sending it to the artisan.' }}</p>
            </div>
            <div class="shrink-0 flex flex-wrap items-center gap-3">
                ...
                <button type="button" id="prop-draft" class="inline-flex items-center gap-2.5 bg-white border border-[#E5E7E5] hover:border-[#14532D] rounded-lg px-4 py-2.5 text-[13px] font-semibold text-[#1B1B18] transition-colors">
                    <i data-lucide="file-text" class="w-4 h-4" style="stroke-width:1.7"></i>
                    <span id="prop-draft-label">{{ $isFr ? 'Enregistrer brouillon' : 'Save draft' }}</span>
                </button>
                <a href="{{ route('quotes.index', ['lang' => $lang, 'tab' => 'propositions']) }}" class="inline-flex items-center gap-2.5 bg-[#0E5A2D] hover:bg-[#14652F] rounded-lg px-5 py-2.5 text-[13px] font-semibold text-white transition-colors">
        ...
        <div class="mt-5 bg-white border border-[#EFF0EF] rounded-2xl px-5 py-4 overflow-x-auto">
            <div class="flex items-center gap-3 min-w-[860px]">
                ...
                <div class="flex items-center gap-3.5 shrink-0">
                    <span class="w-[36px] h-[36px] shrink-0 rounded-full flex items-center justify-center text-[14px] font-bold {{ $stIdx === 3 ? 'bg-[#0E5A2D] text-white' : 'bg-white border border-[#D9DDD9] text-[#55524A]' }}">{{ $stNum }}</span>
                    <span>
                        <span class="block text-[13px] font-bold {{ $stIdx === 3 ? 'text-[#14652F]' : 'text-[#1B1B18]' }}">{{ $stTitle }}</span>
                        <span class="block mt-0.5 text-[11.5px] text-[#6F6B60]">{{ $stSub }}</span>
                    </span>
                </div>
                @if($stIdx < count($steps) - 1)
                <span class="flex-1 min-w-[36px] h-px {{ $stIdx < 3 ? 'bg-[#157A43]' : 'bg-[#E0E4E0]' }}"></span>
                @endif
                @endforeach
            </div>
        </div>
        ...
        <div class="mt-5 flex flex-col xl:flex-row gap-5 items-start">
            ...
            <!-- Left column -->
            <div class="w-full xl:w-[300px] shrink-0 space-y-4">
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-[13.5px] font-bold text-[#1B1B18]">{{ $isFr ? 'Informations de la proposition' : 'Proposal information' }}</h2>
                        <a href="{{ route('quotes.create', ['lang' => $lang]) }}" class="text-[12px] font-semibold text-[#157A43]">{{ $isFr ? 'Modifier' : 'Edit' }}</a>
                    </div>
                    <dl class="mt-4 space-y-3">
                        @foreach($propInfo as [$piLabel, $piValue])
                        <div class="flex items-start justify-between gap-3">
                            <dt class="text-[12px] text-[#55524A]">{{ $piLabel }}</dt>
                            <dd class="text-[12px] font-bold text-[#1B1B18] text-right">{{ $piValue }}</dd>
                        </div>
                        @endforeach
                    </dl>
                </section>
                ...
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-[13.5px] font-bold text-[#1B1B18]">{{ $isFr ? 'Conditions de paiement' : 'Payment terms' }}</h2>
                        <a href="{{ route('quotes.create', ['lang' => $lang]) }}" class="text-[12px] font-semibold text-[#157A43]">{{ $isFr ? 'Modifier' : 'Edit' }}</a>
                    </div>
                    <p class="mt-3 text-[12.5px] text-[#3B382F]">{{ $isFr ? '50% à la commande, 50% avant expédition' : '50% on order, 50% before shipment' }}</p>
                </section>

                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-[13.5px] font-bold text-[#1B1B18]">{{ $isFr ? 'Documents joints' : 'Attached documents' }}</h2>
                        <a href="{{ route('quotes.create', ['lang' => $lang]) }}" class="text-[12px] font-semibold text-[#157A43]">{{ $isFr ? 'Modifier' : 'Edit' }}</a>
                    </div>
                    <div class="mt-3.5 space-y-3">
                        @foreach($propDocs as [$pdIcon, $pdName, $pdSize])
                        <div class="flex items-center gap-3">
                            <img src="{{ asset('images/landing/' . $pdIcon) }}" alt="" class="w-[26px] h-[26px] shrink-0" aria-hidden="true">
                            <span class="flex-1 min-w-0 text-[12px] font-semibold text-[#1B1B18] whitespace-nowrap overflow-hidden text-ellipsis">{{ $pdName }}</span>
                            <span class="shrink-0 text-[11.5px] text-[#6F6B60]">{{ $pdSize }}</span>
                        </div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-[13.5px] font-bold text-[#1B1B18]">{{ $isFr ? 'Message pour l\'artisan' : 'Message for the artisan' }}</h2>
                        <a href="{{ route('quotes.create', ['lang' => $lang]) }}" class="text-[12px] font-semibold text-[#157A43]">{{ $isFr ? 'Modifier' : 'Edit' }}</a>
                    </div>
                    <p class="mt-3 text-[12.5px] text-[#3B382F] leading-relaxed">{{ $isFr ? "Nous recherchons des meubles en bois massif de haute qualité pour l'aménagement de 20 chambres d'hôtel. Style moderne avec une touche traditionnelle camerounaise." : 'We are looking for high-quality solid wood furniture to fit out 20 hotel rooms. Modern style with a traditional Cameroonian touch.' }}</p>
                </section>
            </div>
            ...
            <div class="flex-1 min-w-0 w-full bg-white border border-[#ECEDEC] rounded-2xl shadow-sm px-7 py-7">
                ...
                <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="flex items-start gap-4">
                        <img src="{{ asset('images/landing/qv-abn-logo.png') }}" alt="" class="w-[76px] shrink-0 object-contain">
                    ...
                    <table class="w-full min-w-[760px]">
                        ...
                        <tbody class="divide-y divide-[#F1F2F1]">
                            ...
                            <tr>
                                ...
                                <td class="px-2 pr-3 py-3.5 align-top text-right text-[12px] font-bold text-[#1B1B18] whitespace-nowrap">{{ $dTotal }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="mt-5 border-t border-[#EFF0EF] pt-5 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <h3 class="mt-4 text-[12.5px] font-bold text-[#1B1B18]">Notes</h3>
                    ...
                    <div class="lg:pl-8">
                        ...
                        <div class="mt-3.5 border-t border-[#EFF0EF] pt-3.5 flex items-center justify-between gap-3">
                            <span class="text-[14.5px] font-bold text-[#1B1B18] uppercase">{{ $isFr ? 'Total général' : 'Grand total' }}</span>
                            <span class="text-[15.5px] font-bold text-[#14652F]">5,952,258 FCFA</span>
                        </div>
                    </div>
                </div>

                <div class="mt-6 bg-[#FBFCFB] border border-[#F0F1F0] rounded-xl px-4 py-3.5 flex flex-wrap items-stretch gap-y-3 lg:divide-x divide-[#EAEBEA]">
                    @foreach($docChips as [$dcIcon, $dcLabel, $dcValue])
                    <div class="flex items-center gap-2.5 flex-1 min-w-[170px] lg:px-3.5 first:lg:pl-0 last:lg:pr-0">
                        <i data-lucide="{{ $dcIcon }}" class="w-[19px] h-[19px] shrink-0 text-[#55524A]" style="stroke-width:1.5"></i>
                        <span class="min-w-0">
                            <span class="block text-[11px] font-bold text-[#1B1B18]">{{ $dcLabel }}</span>
                            <span class="block text-[11px] text-[#55524A] whitespace-nowrap">{{ $dcValue }}</span>
                        </span>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="mt-7 flex flex-wrap items-end justify-between gap-6">
                    <p class="text-[12px] text-[#3B382F]">{{ $isFr ? 'Merci pour votre confiance.' : 'Thank you for your trust.' }}</p>
                    ...
                    <div class="flex items-center gap-3.5">
                        <span id="prop-qr" class="inline-block w-[62px] h-[62px]"></span>
            ...
            <!-- Right rail -->
            <aside class="w-full xl:w-[280px] shrink-0 space-y-4">
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <h2 class="text-[14px] font-bold text-[#1B1B18]">{{ $isFr ? 'Résumé financier' : 'Financial summary' }}</h2>
                    <dl class="mt-4 space-y-3">
                        @foreach($financeRows as [$fnLabel, $fnValue, $fnColor, $fnBold])
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-[12px] text-[#3B382F]">{{ $fnLabel }}</dt>
                    ...
                    <div class="mt-4 bg-[#EFF6F1] rounded-xl px-4 py-3.5 flex items-start gap-3">
                        <i data-lucide="badge-percent" class="w-[20px] h-[20px] shrink-0 text-[#1F8A4C]" style="stroke-width:1.7"></i>
                ...
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    ...
                    <div class="mt-2 divide-y divide-[#F1F2F1]">
                        @foreach($sideActions as [$saIcon, $saLabel])
                        <button type="button" onclick="window.print()" class="w-full flex items-center gap-3.5 py-3 group">
                            <i data-lucide="{{ $saIcon }}" class="w-[18px] h-[18px] shrink-0 text-[#3B382F]" style="stroke-width:1.7"></i>
                            <span class="flex-1 text-left text-[13px] text-[#3B382F] group-hover:text-[#14652F]">{{ $saLabel }}</span>
                        </button>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="bg-white border border-[#EFF0EF] rounded-2xl px-5 py-5">
                    <h2 class="text-[14px] font-bold text-[#1B1B18]">{{ $isFr ? 'Options d\'envoi' : 'Sending options' }}</h2>
                    <div class="mt-3.5 space-y-3">
                        @foreach([$isFr ? 'Envoyer par email' : 'Send by email', $isFr ? 'Partager un lien sécurisé' : 'Share a secure link', $isFr ? 'Envoyer plus tard' : 'Send later'] as $soIdx => $soLabel)
                        <label class="flex items-center gap-3 cursor-pointer select-none">
                            <input type="radio" name="send_option" {{ $soIdx === 0 ? 'checked' : '' }} class="w-4 h-4 accent-[#14652F]">
                            <span class="text-[12.5px] text-[#3B382F]">{{ $soLabel }}</span>
                        </label>
                        @endforeach
                    </div>
                    <a href="{{ route('quotes.index', ['lang' => $lang, 'tab' => 'propositions']) }}" class="mt-5 flex items-center justify-center gap-3 bg-[#0E5A2D] hover:bg-[#14652F] rounded-xl px-4 py-3.5 text-white transition-colors">
                        <i data-lucide="plus" class="w-4 h-4"></i>
                        <span class="text-left leading-snug">
                            <span class="block text-[13px] font-bold">{{ $isFr ? 'Étape suivante' : 'Next step' }}</span>
                            <span class="block text-[11px] text-white/80">{{ $isFr ? 'Envoyer cette proposition' : 'Send this proposal' }}</span>
                        </span>
...
<script>
    ...
    // Live QR encoding this proposal's verification URL
    new QRCode(document.getElementById('prop-qr'), {
        text: @json(route('quotes.proposal', ['lang' => $lang]) . '?devis=QUO-2024-000189'),
        width: 62, height: 62, correctLevel: QRCode.CorrectLevel.M,
    });

    document.getElementById('prop-draft').addEventListener('click', () => {
        localStorage.setItem('propDraft', 'QUO-2024-000189');
        document.getElementById('prop-draft-label').textContent = @json($isFr ? 'Brouillon enregistré ✓' : 'Draft saved ✓');
    });
</script>
</body>
</html>
