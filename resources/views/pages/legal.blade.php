...
<div class="max-w-3xl mx-auto px-4 py-8 sm:py-12">
    ...
    <h1 class="text-[24px] sm:text-[28px] font-bold text-[#1B1B18] leading-tight">{{ $doc['title'][$lang] }}</h1>
    <p class="mt-1.5 text-[12px] text-[#A8A296]">
        {{ $isFr ? 'Dernière mise à jour :' : 'Last updated:' }} {{ $updatedAt }}
    </p>

    @if(! empty($doc['intro'][$lang]))
    <p class="mt-5 text-[14px] text-[#3B382F] leading-relaxed">{{ $doc['intro'][$lang] }}</p>
    @endif

    <div class="mt-8 space-y-7">
        @foreach($doc['sections'] as $i => $section)
        <section>
            <h2 class="text-[15px] font-bold text-[#1B1B18] mb-2">{{ $i + 1 }}. {{ $section['heading'][$lang] }}</h2>
            @foreach((array) $section['body'][$lang] as $para)
            <p class="text-[13.5px] text-[#55524A] leading-relaxed mb-2 last:mb-0">{{ $para }}</p>
            @endforeach
            @if(! empty($section['list'][$lang]))
            <ul class="mt-2 space-y-1.5">
                @foreach($section['list'][$lang] as $item)
                <li class="flex gap-2.5 text-[13.5px] text-[#55524A] leading-relaxed">
    ...
    <div class="mt-10 rounded-xl border border-[#F1E7D2] bg-[#FBF6EC] px-5 py-4">
        <p class="flex items-center gap-2 text-[12.5px] font-bold text-[#8A6D1F]">
    ...
    <div class="mt-8 pt-6 border-t border-[#ECECEA] flex flex-wrap items-center gap-4">
        <p class="text-[12.5px] text-[#8A857A]">
            {{ $isFr ? 'Une question sur ce document ?' : 'A question about this document?' }}
        </p>
        <a href="{{ route('contact', ['lang' => $lang]) }}" class="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#14652F] hover:text-[#14532D]">
