            ...
            <div class="mt-4 grid grid-cols-1 2xl:grid-cols-[1fr_320px] gap-5 items-start">
                <div class="space-y-5">
                    ...
                    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0E3D22] dark:from-[#0C3B1E] to-[#12522C] dark:to-[#2E9250] px-6 py-5">
                        <span class="inline-block rounded-md px-2.5 py-0.5 text-[11px] font-semibold bg-[#E9C25A]/20 dark:bg-[#3A2B06] text-[#E9C25A]">{{ $isFr?'Envoyée':'Sent' }}</span>
                        <div class="mt-2 flex flex-wrap items-start justify-between gap-3">
                            <div class="flex items-center gap-3"><span class="w-12 h-12 rounded-xl bg-white/10 dark:bg-[#12150F]/10 flex items-center justify-center"><i data-lucide="bell" class="w-6 h-6 text-[#E9C25A]"></i></span><div><h1 class="text-[19px] font-bold text-white">{{ $notification->title }}</h1><p class="text-[12.5px] text-[#CFE3D5]">{{ $notification->body }}</p></div></div>
                            <div class="flex items-center gap-2.5">
                                <span class="inline-flex items-center gap-2 bg-white/10 dark:bg-[#12150F]/10 rounded-lg px-3.5 h-[36px] text-[12px] font-semibold text-white"><i data-lucide="settings" class="w-4 h-4"></i>Actions</span>
                                <a href="{{ route('admin.cms', ['lang'=>$lang]) }}" class="inline-flex items-center gap-2 bg-white/10 dark:bg-[#12150F]/10 hover:bg-white/15 dark:hover:bg-[#242A1E]/15 rounded-lg px-3.5 h-[36px] text-[12px] font-semibold text-white"><i data-lucide="send" class="w-4 h-4"></i>{{ $isFr?'Renvoyer':'Resend' }}</a>
                            </div>
                        </div>
