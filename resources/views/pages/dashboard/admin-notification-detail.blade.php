            ...
            <div class="mt-4 grid grid-cols-1 2xl:grid-cols-[1fr_320px] gap-5 items-start">
                <div class="space-y-5">
                    ...
                    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0E3D22] dark:from-[#0C3B1E] to-[#12522C] dark:to-[#2E9250] px-6 py-5">
                        <span class="inline-block rounded-md px-2.5 py-0.5 text-[13px] md:text-[11px] font-semibold bg-[#E9C25A]/20 dark:bg-[#3A2B06] text-[#E9C25A]">{{ $isFr?'Envoyée':'Sent' }}</span>
                        <div class="mt-2 flex flex-wrap items-start justify-between gap-3">
                            <div class="flex items-center gap-3"><span class="w-12 h-12 rounded-xl bg-white/10 dark:bg-[#12150F]/10 flex items-center justify-center"><i data-lucide="bell" class="w-6 h-6 text-[#E9C25A]"></i></span><div><h1 class="text-[19px] font-bold text-white">{{ $notification->title }}</h1><p class="text-[14px] md:text-[12.5px] text-[#CFE3D5]">{{ $notification->body }}</p></div></div>
                            <div class="flex items-center gap-2.5">
                                <span class="inline-flex items-center gap-2 bg-white/10 dark:bg-[#12150F]/10 rounded-lg px-3.5 h-11 md:h-[36px] text-[14px] md:text-[12px] font-semibold text-white"><i data-lucide="settings" class="w-4 h-4"></i>Actions</span>
                                <a href="{{ route('admin.cms', ['lang'=>$lang]) }}" class="inline-flex items-center gap-2 bg-white/10 dark:bg-[#12150F]/10 hover:bg-white/15 dark:hover:bg-[#242A1E]/15 rounded-lg px-3.5 h-11 md:h-[36px] text-[14px] md:text-[12px] font-semibold text-white"><i data-lucide="send" class="w-4 h-4"></i>{{ $isFr?'Renvoyer':'Resend' }}</a>
                            </div>
                        </div>
                        <p class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 text-[13px] md:text-[11.5px] text-[#CFE3D5]"><span class="flex items-center gap-1.5"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>{{ $dtf($notification->created_at) }}</span>@if($recipient)<span class="flex items-center gap-1.5"><i data-lucide="user" class="w-3.5 h-3.5"></i>{{ $isFr?'Destinataire':'Recipient' }} : {{ $recipient->name }}</span>@endif<span class="flex items-center gap-1.5"><i data-lucide="bell" class="w-3.5 h-3.5"></i>{{ $isFr?'Canal : Notification in-app':'Channel: in-app notification' }}</span><span class="flex items-center gap-1.5"><i data-lucide="hash" class="w-3.5 h-3.5"></i>{{ $ref }}</span></p>
                    </section>

                    {{-- Content --}}
                    <section class="ui-card">
                        <h2 class="ui-card-title">{{ $isFr?'Contenu de la notification':'Notification content' }}</h2>
                        <dl class="mt-4 space-y-3 text-[14px] md:text-[12.5px]">
                            ...
                            <div class="flex gap-4"><dt class="w-28 shrink-0 text-[#6F6B60] dark:text-[#868778]">{{ $isFr?'Lien':'Link' }} :</dt><dd><a href="{{ $notification->link }}" class="text-[14px] md:text-[12px] text-[#3565DE] dark:text-[#8FB6F5] underline break-all">{{ url($notification->link) }}</a></dd></div>
                            @endif
                        </dl>
                    </section>
                    ...
                    <section class="ui-card">
                        ...
                        <div class="mt-4 border border-[#EFEBE2] dark:border-[#262B21] rounded-xl overflow-hidden">
                            <div class="bg-gradient-to-r from-[#0E3D22] dark:from-[#0C3B1E] to-[#12522C] dark:to-[#2E9250] px-6 py-4 flex items-center gap-3"><img src="{{ brand_asset('mark') }}" alt="" class="w-10 h-10 object-contain"><div><p class="text-[13px] md:text-[11px] font-bold text-white uppercase leading-tight">Artisan Hub 237<br>de l'Artisanat du Cameroun</p></div></div>
                            <div class="px-6 py-5"><p class="text-[13px] text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr?'Bonjour,':'Hello,' }}</p><p class="mt-2 text-[14px] md:text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $notification->body }}</p><div class="mt-4 flex items-center justify-between bg-[#F7F8F7] dark:bg-[#12150F] rounded-lg px-4 py-3"><span class="text-[13px] md:text-[11.5px] text-[#6F6B60] dark:text-[#868778]">{{ $notification->title }}</span>@if($notification->link)<a href="{{ $notification->link }}" class="bg-[#0F4824] dark:bg-[#2E9250] text-white dark:text-[#04150A] rounded-lg px-3.5 py-1.5 min-h-[44px] md:min-h-0 inline-flex items-center text-[13px] md:text-[11.5px] font-semibold">{{ $isFr?'Voir':'View' }}</a>@endif</div><p class="mt-4 text-[14px] md:text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr?'Merci,':'Thank you,' }}<br>{{ $isFr?'Équipe Support – Artisan Hub 237':'Support Team – Artisan Hub 237' }}</p></div>
                        </div>
                    </section>
                </div>
                ...
                <aside class="space-y-4">
                    <section class="ui-card">
                        ...
                        <dl class="mt-3 space-y-2.5 text-[14px] md:text-[12px]">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <dd class="text-right"><span class="inline-block rounded-md px-2.5 py-0.5 text-[13px] md:text-[11px] font-semibold {{ $isRead ? 'bg-[#E4F1E8] dark:bg-[#0C3D1D] text-[#157A43] dark:text-[#339B56] ' : 'bg-[#FBF1DD] dark:bg-[#3A2B06] text-[#C9942E] dark:text-[#EDB33A] ' }}">{{ $isRead ? ($isFr?'Lue':'Read') : ($isFr?'Non lue':'Unread') }}</span></dd>
                            </div>
                            @if($isRead)
                    ...
                    <section class="ui-card">
                        ...
                        <dl class="mt-3 space-y-2.5 text-[14px] md:text-[12px]">@foreach($infos as [$l,$v])<div class="flex items-center justify-between gap-3"><dt class="text-[#6F6B60] dark:text-[#868778]">{{ $l }}</dt><dd class="font-semibold text-[#1B1B18] dark:text-[#F3EFE7] text-right">{{ $v }}</dd></div>@endforeach</dl>
                    </section>
                    <section class="ui-card">
                        <h2 class="ui-card-title">Audit</h2>
                        <div class="mt-3 space-y-3">@foreach($audit as [$aE,$aW])<div class="flex gap-3"><span class="w-7 h-7 rounded-full bg-[#F3F0E6] dark:bg-[#1A1E16] flex items-center justify-center shrink-0"><i data-lucide="clock" class="w-3.5 h-3.5 text-[#14652F] dark:text-[#339B56]"></i></span><div><p class="text-[14px] md:text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $aE }}</p><p class="text-[12px] md:text-[10.5px] text-[#8A857A] dark:text-[#868778]">{{ $aW }}</p></div></div>@endforeach</div>
                    </section>
                </aside>
            </div>
