...
<div class="max-w-5xl">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-[#1B1B18]">{{ $stats['businesses'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Entreprises' : 'Businesses' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-[#1B1B18]">{{ $stats['products'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Produits' : 'Products' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-[#1B1B18]">{{ $stats['regions'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Régions' : 'Regions' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-[#1B1B18]">{{ $stats['industries'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Secteurs' : 'Industries' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-amber-600">{{ $stats['certified'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Certifiées' : 'Certified' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 text-center">
            <p class="text-2xl font-bold text-green-600">{{ $stats['verified'] }}</p>
            <p class="text-xs text-[#8A857A] mt-1">{{ $lang === 'fr' ? 'Vérifiées' : 'Verified' }}</p>
        </div>
    </div>

    <!-- Growth chart -->
    <div class="bg-white rounded-xl border border-[#EFEBE2] p-4 mb-4">
        <h2 class="text-sm font-semibold text-[#1B1B18] mb-4">{{ $lang === 'fr' ? 'Croissance — nouvelles entreprises (6 derniers mois)' : 'Growth — new businesses (last 6 months)' }}</h2>
        <div class="flex items-end gap-3 h-32">
            @forelse($growth as $point)
                @php $m = substr($point->month, 5, 2); @endphp
                <div class="flex-1 flex flex-col items-center gap-1.5">
                    <span class="text-xs font-semibold text-[#3B382F]">{{ $point->total }}</span>
                    <div class="w-full bg-forest-500 rounded-t-md" style="height: {{ max(6, round($point->total / $maxGrowth * 96)) }}px"></div>
                    <span class="text-[10px] text-[#A8A296]">{{ $monthLabels[$m][$lang] ?? $m }}</span>
                </div>
            @empty
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div class="bg-white rounded-xl border border-[#EFEBE2] p-4">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-3">{{ $lang === 'fr' ? 'Par région' : 'By region' }}</h2>
            <div class="space-y-2">
    ...
    <div class="bg-white rounded-xl border border-[#EFEBE2] overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-4 py-3 border-b border-[#FBF9F4] last:border-0">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-[#A8A296]">{{ $event->starts_at?->format('d/m/Y') }} — {{ $lang === 'fr' ? $event->location_fr : $event->location_en }}</p>
            </div>
            <div class="flex items-center gap-4 shrink-0 text-xs text-[#8A857A]">
        ...
        @empty
        <div class="text-center py-8 text-sm text-[#A8A296]">{{ $lang === 'fr' ? 'Aucun événement enregistré.' : 'No events recorded.' }}</div>
        @endforelse
    </div>
</div>
@endsection
