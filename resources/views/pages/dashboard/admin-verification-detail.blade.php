...
@section('content')
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h1 class="flex items-center gap-3 text-[22px] md:text-[20px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr?'DÉTAIL DE VÉRIFICATION ARTISAN':'ARTISAN VERIFICATION DETAIL' }} <span class="rounded-md px-2.5 py-0.5 text-[13px] md:text-[11px] font-semibold" style="background-color:{{ $stColor }}1a;color:{{ $stColor }}">{{ $stLabel }}</span></h1>
                    <p class="mt-0.5 text-[14px] md:text-[12px] text-[#6F6B60] dark:text-[#868778]">ID {{ $isFr?'Demande':'Request' }} : {{ $ref }}</p>
                </div>
                ...
                <div class="space-y-5">
                    ...
                    <section class="ui-card flex flex-wrap items-start gap-5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="mt-2 text-[14px] md:text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]"><i data-lucide="briefcase" class="inline w-4 h-4 text-[#8A857A] dark:text-[#868778]"></i> {{ $isFr?'Spécialité':'Specialty' }} : {{ $app->industry_fr ?? '—' }}</p>
                            <p class="mt-1 text-[14px] md:text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]"><i data-lucide="map-pin" class="inline w-4 h-4 text-[#8A857A] dark:text-[#868778]"></i> {{ $app->region_fr ?? 'Cameroun' }}, Cameroun</p>
                            <p class="mt-1 text-[14px] md:text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6]"><i data-lucide="mail" class="inline w-4 h-4 text-[#8A857A] dark:text-[#868778]"></i> {{ $app->owner_email ?? '—' }}</p>
                        </div>
                    ...
                    <section class="ui-card">
                        ...
                        <div class="mt-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                            ...
                            <div class="border border-[#EFEBE2] dark:border-[#262B21] rounded-xl overflow-hidden">
                                ...
                                <div class="p-2.5"><p class="text-[13px] md:text-[11px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] leading-tight line-clamp-2">{{ $doc->original_name }}</p><p class="mt-1.5 flex items-center gap-1 text-[12px] md:text-[10.5px] font-semibold" style="color:{{ $dsC }}"><i data-lucide="{{ $dsI }}" class="w-3 h-3"></i>{{ $dsL }}</p></div>
                            </div>
                            @empty
                            <p class="col-span-full text-[14px] md:text-[12px] text-[#6F6B60] dark:text-[#868778]">{{ $isFr?'Aucun document soumis.':'No documents submitted.' }}</p>
                            @endforelse
                        </div>
                        <a href="{{ route('admin.kyc', ['lang'=>$lang]) }}" class="mt-4 inline-flex items-center gap-1.5 min-h-[44px] md:min-h-0 text-[14px] md:text-[12px] font-semibold text-[#157A43] dark:text-[#339B56]"><i data-lucide="eye" class="w-4 h-4"></i>{{ $isFr?'Voir tous les documents':'View all documents' }}</a>
                    </section>
                </div>
                ...
                <aside class="space-y-4">
                    <section class="ui-card">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <div class="flex items-center gap-3"><span class="w-7 h-7 rounded-full flex items-center justify-center text-[13px] md:text-[11px] font-bold {{ $sDone ? 'bg-[#157A43] dark:bg-[#2E9250] text-white dark:text-[#04150A]' : 'bg-[#F0EFEA] dark:bg-[#1A1E16] text-[#8A857A] dark:text-[#868778] ' }}">{{ $sDone ? '' : ($i+1) }}@if($sDone)<i data-lucide="check" class="w-3.5 h-3.5"></i>@endif</span><span class="text-[14px] md:text-[12px] {{ $sDone?'font-semibold text-[#1B1B18] dark:text-[#F3EFE7]':'text-[#6F6B60] dark:text-[#868778] ' }}">{{ $sLabel }}</span></div>
                            @endforeach
                        </div>
                    </section>
                    <section class="ui-card">
                        <h2 class="ui-card-title">{{ $isFr?'Résumé de la vérification':'Verification summary' }}</h2>
                        <dl class="mt-3.5 space-y-2.5 text-[14px] md:text-[12px]">@foreach($resume as [$l,$v,$c])<div class="flex items-center justify-between gap-3"><dt class="text-[#6F6B60] dark:text-[#868778]">{{ $l }}</dt><dd class="inline-flex items-center gap-1 font-semibold" style="color:{{ $c }}"><i data-lucide="{{ $c==='#157A43'?'check-circle-2':'clock' }}" class="w-3.5 h-3.5"></i>{{ $v }}</dd></div>@endforeach</dl>
                    </section>
                    <section class="ui-card">
                        <h2 class="ui-card-title">{{ $isFr?'Notes de l\'agent':'Agent notes' }}</h2>
                        <p class="mt-3 text-[14px] md:text-[12px] text-[#3B382F] dark:text-[#B4B5A6] leading-relaxed">{{ $app->reviewer_notes ?? ($isFr?'Documents d\'identité valides. Vérification d\'identité en cours. L\'atelier semble authentique, visite recommandée.':'Valid ID documents. Identity check in progress. The workshop looks authentic, a visit is recommended.') }}</p>
                    </section>
                </aside>
            </div>
