...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
        ...
        @empty
        <div class="text-center py-10 text-sm text-gray-400">{{ $lang === 'fr' ? 'Aucun ticket pour l\'instant.' : 'No tickets yet.' }}</div>
        @endforelse
    </div>

    <div class="bg-white border border-gray-200 rounded-xl p-5">
        <h2 class="text-sm font-semibold text-gray-900 mb-4">{{ $lang === 'fr' ? 'Nouveau ticket' : 'New ticket' }}</h2>
        <form method="POST" action="{{ route('support.store') }}" class="space-y-3">
            @csrf
            @if($categories->isNotEmpty())
            <select name="category_id" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-forest-400">
            ...
            @endif
            <input name="subject" required maxlength="255" placeholder="{{ $lang === 'fr' ? 'Sujet' : 'Subject' }}" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-forest-400">
            <textarea name="body" required rows="4" maxlength="3000" placeholder="{{ $lang === 'fr' ? 'Décrivez votre problème...' : 'Describe your issue...' }}" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-forest-400 resize-none"></textarea>
