...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-100">
            ...
            <h2 class="text-sm font-semibold text-gray-900">{{ $lang === 'fr' ? 'Informations' : 'Information' }}</h2>
        </div>
        <form method="POST" action="{{ route('profile.update') }}" class="p-5 space-y-4">
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required maxlength="255"
                    class="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-forest-400 focus:ring-1 focus:ring-forest-400 transition">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5" for="language_preference">
                    {{ $lang === 'fr' ? 'Langue préférée' : 'Preferred language' }}
                </label>
                <select id="language_preference" name="language_preference"
                    class="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-forest-400 focus:ring-1 focus:ring-forest-400 transition bg-white">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input type="email" value="{{ $user->email }}" disabled
                        class="w-full px-3.5 py-2.5 border border-gray-200 bg-gray-50 rounded-lg text-sm text-gray-500 cursor-not-allowed">
                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 text-xs font-semibold whitespace-nowrap">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-100">
            ...
            <h2 class="text-sm font-semibold text-gray-900">{{ $lang === 'fr' ? 'Mot de passe' : 'Password' }}</h2>
        </div>
        <form method="POST" action="{{ route('profile.password') }}" class="p-5 space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5" for="current_password">
