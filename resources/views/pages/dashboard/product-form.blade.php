...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="bg-white border border-[#EFEBE2] rounded-xl p-5">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-4">{{ $lang === 'fr' ? 'Informations générales' : 'General information' }}</h2>
        ...
        <div class="bg-white border border-[#EFEBE2] rounded-xl p-5">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-4">{{ $lang === 'fr' ? 'Quantité & Commande' : 'Quantity & Order' }}</h2>
        ...
        <div class="bg-white border border-[#EFEBE2] rounded-xl p-5">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1">{{ $lang === 'fr' ? 'Type de prix' : 'Price type' }}</h2>
            <p class="text-xs text-[#A8A296] mb-4">{{ $lang === 'fr' ? 'Le prix exact n\'est jamais affiché publiquement — seul le type est visible, les acheteurs doivent vous contacter.' : 'The exact price is never shown publicly — only the type is visible, buyers must contact you.' }}</p>
        ...
        <div class="bg-white border border-[#EFEBE2] rounded-xl p-5">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-4">{{ $lang === 'fr' ? 'Caractéristiques' : 'Characteristics' }}</h2>
        ...
        <div class="bg-white border border-[#EFEBE2] rounded-xl p-5">
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-4">{{ $lang === 'fr' ? 'Photos' : 'Photos' }}</h2>
            ...
            <label class="block text-xs font-medium text-[#6F6B60] mb-1">{{ $lang === 'fr' ? 'Ajouter des photos (jusqu\'à 8)' : 'Add photos (up to 8)' }}</label>
            <input type="file" name="images[]" accept="image/*" multiple class="w-full text-xs text-[#8A857A] file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-forest-50 file:text-forest-700 file:text-xs">
        </div>
