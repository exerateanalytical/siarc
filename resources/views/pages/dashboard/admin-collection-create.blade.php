...
@section('content')
            <form method="POST" action="{{ route('admin.collections.store') }}" enctype="multipart/form-data">
                ...
                {{-- Tabs --}}
                <div class="mt-4 flex items-center gap-6 border-b border-[#EFEBE2] overflow-x-auto">
                ...
                <div class="mt-5 grid grid-cols-1 xl:grid-cols-[1fr_330px] gap-5 items-start">
                    ...
                    <section class="ui-card">
                        ...
                        <div class="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div>
                                ...
                                <div class="ui-field-group"><span class="px-3 bg-[#F5F3EE] text-[12px] text-[#8A857A] h-full flex items-center border-r border-[#EAE5D8]">collections/</span><input type="text" name="slug" value="{{ old('slug') }}" placeholder="{{ $isFr?'ex: masques-traditionnels-cameroun':'ex: traditional-masks' }}" class="ui-field-bare flex-1 min-w-0"></div>
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <div>
                                    <div class="flex items-center gap-1 mb-1.5 text-[#8A857A]">@foreach(['bold','italic','underline','list','link','quote'] as $tb)<span class="w-7 h-7 rounded flex items-center justify-center hover:bg-[#F5F3EE]"><i data-lucide="{{ $tb }}" class="w-3.5 h-3.5"></i></span>@endforeach</div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <label class="flex-1 cursor-pointer"><input type="radio" name="status" value="in_review" class="peer sr-only"><span class="block text-center rounded-lg border border-[#EAE5D8] py-2 text-[12px] font-semibold text-[#8A857A] peer-checked:bg-[#F5F3EE]">{{ $isFr?'En attente':'In review' }}</span></label>
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    <label class="ui-check-row items-center cursor-pointer"><input type="radio" name="visibility" value="public" class="ui-check" checked><span class="text-[12.5px]"><b>Public</b> <span class="text-[#8A857A]">— {{ $isFr?'Visible par tous les visiteurs':'Visible to all visitors' }}</span></span></label>
                        ...
                        <div class="mt-6 flex items-center justify-end gap-3 border-t border-[#EFEBE2] pt-4">
                    ...
                    <aside class="space-y-4">
                        <section class="ui-card">
                            ...
                            <label class="mt-1 block border-2 border-dashed border-[#EAE5D8] rounded-xl px-4 py-8 text-center cursor-pointer hover:border-[#C9942E]">
                        ...
                        <section class="ui-card">
                            ...
                            <div class="mt-3 space-y-3">
                                <label class="ui-check-row items-start cursor-pointer"><input type="checkbox" name="featured" checked class="ui-check mt-0.5"><span class="text-[12px]"><b class="text-[#1B1B18]">{{ $isFr?'Collection mise en avant':'Featured collection' }}</b><span class="block text-[10.5px] text-[#8A857A]">{{ $isFr?'Afficher cette collection sur la page d\'accueil':'Show this collection on the homepage' }}</span></span></label>
                                <label class="ui-check-row items-start cursor-pointer"><input type="checkbox" name="comments" class="ui-check mt-0.5"><span class="text-[12px]"><b class="text-[#1B1B18]">{{ $isFr?'Autoriser les commentaires':'Allow comments' }}</b></span></label>
                                <label class="ui-check-row items-start cursor-pointer"><input type="checkbox" name="searchable" checked class="ui-check mt-0.5"><span class="text-[12px]"><b class="text-[#1B1B18]">{{ $isFr?'Inclure dans la recherche':'Include in search' }}</b></span></label>
                                <label class="ui-check-row items-start cursor-pointer"><input type="checkbox" name="sponsored" class="ui-check mt-0.5"><span class="text-[12px]"><b class="text-[#1B1B18]">{{ $isFr?'Collection sponsorisée':'Sponsored collection' }}</b></span></label>
                            </div>
