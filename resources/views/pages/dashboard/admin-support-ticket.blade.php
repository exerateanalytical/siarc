            ...
            <div class="grid grid-cols-1 2xl:grid-cols-[1fr_320px] gap-5 items-start">
                <div class="space-y-5">
                    ...
                    <section class="ui-card">
                        ...
                        <div class="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 border-t border-[#EFEBE2] dark:border-[#262B21] pt-4">
                            @foreach($metaRow as [$mI,$mL,$mV])<div class="flex items-start gap-2"><i data-lucide="{{ $mI }}" class="w-4 h-4 mt-0.5 text-[#C9942E] dark:text-[#EDB33A] shrink-0"></i><div><p class="text-[10px] text-[#8A857A] dark:text-[#868778]">{{ $mL }}</p><p class="text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $mV }}</p></div></div>@endforeach
                        </div>
                    </section>
                    ...
                    <section class="ui-card">
                        ...
                        <div class="mt-4 space-y-5">
                            ...
                            <div class="flex gap-3">
                                <span class="w-9 h-9 rounded-full {{ $staff?'bg-[#C9942E] dark:bg-[#3A2B06] ':'bg-[#14652F] dark:bg-[#2E9250] ' }} text-white text-[12px] font-bold flex items-center justify-center shrink-0">{{ mb_strtoupper(mb_substr($r->author_name ?? 'U',0,2)) }}</span>
                                <div class="min-w-0 flex-1">
                                    <p class="flex flex-wrap items-center gap-2 text-[13px]"><b class="text-[#1B1B18] dark:text-[#F3EFE7]">{{ $r->author_name ?? ($staff?'Agent':'Client') }}</b><span class="rounded px-1.5 py-0.5 text-[10px] font-semibold {{ $staff?'bg-[#FDF3E0] dark:bg-[#3A2B06] text-[#C97A16] dark:text-[#EDB33A] ':'bg-[#E2F3E8] dark:bg-[#0C3D1D] text-[#157A43] dark:text-[#339B56] ' }}">{{ $staff?'Agent':'Client' }}</span><span class="ml-auto text-[11px] text-[#8A857A] dark:text-[#868778]">{{ $dtf($r->created_at) }}</span></p>
                                    <div class="mt-1.5 text-[12.5px] text-[#3B382F] dark:text-[#B4B5A6] leading-relaxed whitespace-pre-line">{{ $isFr ? $r->body_fr : ($r->body_en ?? $r->body_fr) }}</div>
                                </div>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <form method="POST" action="{{ route('admin.support.reply', ['id'=>$ticket->id]) }}" id="reply" class="mt-6 border-t border-[#EFEBE2] dark:border-[#262B21] pt-4">
                            ...
                            @error('body')<p class="mb-2 text-[12px] text-[#B42025] dark:text-[#F0555C]">{{ $message }}</p>@enderror
                ...
                <aside class="space-y-4">
                    ...
                    <section class="ui-card">
                        ...
                        <dl class="mt-3 space-y-2 text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">
                        ...
                        <a href="{{ route('admin.users', ['lang'=>$lang]) }}" class="mt-3 block text-center border border-[#EAE5D8] dark:border-[#262B21] hover:border-[#14652F] dark:hover:border-[#2E9250] rounded-lg py-2 text-[12px] font-semibold text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr?'Voir le profil':'View profile' }}</a>
                    </section>
                </aside>
            </div>
