...
@section('content')
            ...
            <section class="mt-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
                ...
                <div class="ui-card flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-[12px] md:text-[10.5px] font-bold tracking-[0.05em] text-[#8A857A] dark:text-[#868778] uppercase leading-snug">{{ $hcLabel }}</p>
                    ...
                    <section class="ui-card ui-card--flush">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="flex items-center gap-3 min-w-[210px]">
                                                ...
                                                @endif
                                                <div class="min-w-0">
                                                    <p class="text-[15px] md:text-[12.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] leading-snug">{{ $isFr ? $c->name_fr : ($c->name_en ?? $c->name_fr) }}</p>
                                                    <p class="text-[13px] md:text-[11px] text-[#8A857A] dark:text-[#868778] truncate">{{ $isFr ? $c->description_fr : ($c->description_en ?? $c->description_fr) }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <p class="text-[14px] md:text-[12px] font-semibold text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr ? $c->region_fr : ($c->region_en ?? $c->region_fr) }}</p>
                                            <p class="text-[13px] md:text-[11px] text-[#8A857A] dark:text-[#868778]">{{ $c->city }}</p>
                                        </td>
                                        ...
                                        <td>
                                            <div class="flex items-center">
                                                ...
                                                @endforeach
                                                <span class="-ml-1.5 min-w-[26px] h-[22px] px-1 rounded-full bg-[#F1EDE2] dark:bg-[#1A1E16] border-2 border-white text-[12px] md:text-[10px] font-bold text-[#55524A] dark:text-[#B4B5A6] flex items-center justify-center">+{{ $c->artisans_count }}</span>
                                            </div>
                                        </td>
                                        <td class="text-center font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $c->products_count }}</td>
                                        <td class="text-center"><span class="inline-block rounded-md px-2.5 py-1 text-[12px] md:text-[10.5px] font-semibold whitespace-nowrap {{ $hcStatusCls }}">{{ $hcStatusLabel }}</span></td>
                                        <td>
                                            <span class="inline-flex items-center gap-1.5 text-[14px] md:text-[12px] text-[#3B382F] dark:text-[#B4B5A6] whitespace-nowrap">
                        ...
                        <div class="px-5 py-3.5 border-t border-[#F5F1E8] dark:border-[#262B21] flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="w-[28px] h-[28px] rounded-lg bg-[#0F4824] dark:bg-[#2E9250] text-white dark:text-[#04150A] text-[13px] md:text-[11.5px] font-bold flex items-center justify-center">1</span>
                                ...
                                <span class="ml-2 h-[28px] rounded-lg border border-[#EAE5D8] dark:border-[#262B21] px-2.5 text-[13px] md:text-[11.5px] text-[#55524A] dark:text-[#B4B5A6] flex items-center">10 / page</span>
                            </div>
                        </div>
                    </section>
                    ...
                    <div class="mt-4 grid grid-cols-1 lg:grid-cols-[1.35fr_1fr] gap-4 items-start">
                        <section class="ui-card">
                            ...
                            <div class="mt-4 grid grid-cols-2 xl:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    <p class="mt-2.5 text-[12px] md:text-[10px] font-bold tracking-[0.05em] text-[#8A857A] dark:text-[#868778] uppercase">{{ $isFr ? 'Meilleure collection' : 'Best collection' }}</p>
                                    @if($hcBest)
                                    <p class="mt-1 text-[15px] md:text-[12.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] leading-snug">{{ $isFr ? $hcBest->name_fr : ($hcBest->name_en ?? $hcBest->name_fr) }}</p>
                                    <p class="mt-0.5 text-[13px] md:text-[11px] text-[#6F6B60] dark:text-[#868778]">{{ number_format($hcBest->visits_count) }} {{ $isFr ? 'visites' : 'visits' }}</p>
                                    @else
                                    <p class="mt-1 text-[15px] md:text-[12.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">—</p>
                                    @endif
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="mt-2.5 text-[12px] md:text-[10px] font-bold tracking-[0.05em] text-[#8A857A] dark:text-[#868778] uppercase">{{ $isFr ? 'Visites totales' : 'Total visits' }}</p>
                                ...
                                <div>
                                    ...
                                    <p class="mt-2.5 text-[12px] md:text-[10px] font-bold tracking-[0.05em] text-[#8A857A] dark:text-[#868778] uppercase">{{ $isFr ? 'Artisans mis en avant' : 'Featured artisans' }}</p>
                                ...
                                <div>
                                    ...
                                    <p class="mt-2.5 text-[12px] md:text-[10px] font-bold tracking-[0.05em] text-[#8A857A] dark:text-[#868778] uppercase">{{ $isFr ? 'Collections publiées' : 'Published collections' }}</p>
                        ...
                        <section class="ui-card">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <a href="{{ route('admin.audit-log') }}" class="shrink-0 text-[13px] md:text-[11px] font-semibold text-[#C97A16] dark:text-[#EDB33A]">{{ $isFr ? 'Voir toutes' : 'See all' }} →</a>
                            </div>
                            @php
                            ...
                            <ul class="mt-3 divide-y divide-[#F5F1E8] dark:divide-[#262B21]">
                                ...
                                <li class="py-2.5 flex items-start gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-[14px] md:text-[12px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] leading-snug">{{ $hcATitle }}</p>
                                        <p class="text-[13px] md:text-[11px] text-[#6F6B60] dark:text-[#868778] truncate">{{ $hcASub }}</p>
                                    </div>
                                    <span class="shrink-0 text-[12px] md:text-[10.5px] text-[#8A857A] dark:text-[#868778] whitespace-nowrap">{{ $hcAWhen }}</span>
                                </li>
                                @empty
                                <li class="py-2.5 text-[13px] md:text-[11.5px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Aucune activité récente.' : 'No recent activity.' }}</li>
                                @endforelse
                            </ul>
                        </section>
                    </div>
                ...
                <div class="space-y-4">
                        ...
                        <div class="mt-4 flex items-center gap-4">
                            ...
                            <ul class="flex-1 min-w-0 space-y-1.5">
                                ...
                                <li class="flex items-center gap-2">
                                    ...
                                    <span class="flex-1 min-w-0 truncate text-[13px] md:text-[11px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr ? $hcCat : ($hcCatEn[$hcCat] ?? $hcCat) }}</span>
                                    <span class="shrink-0 text-[13px] md:text-[11px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $hcCount }} ({{ number_format($hcCount / $hcDonutTotal * 100, 1) }}%)</span>
                                </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <section class="ui-card">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <a href="{{ route('admin.regions') }}" class="shrink-0 text-[13px] md:text-[11px] font-semibold text-[#157A43] dark:text-[#339B56]">{{ $isFr ? 'Voir le rapport' : 'View report' }} →</a>
                        </div>
                    ...
                    <section class="ui-card">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            @if($hcFeatured)<a href="{{ route('admin.businesses.detail', ['id' => $hcFeatured->id, 'lang' => $lang]) }}" class="shrink-0 text-[13px] md:text-[11px] font-semibold text-[#C97A16] dark:text-[#EDB33A]">{{ $isFr ? 'Voir le profil' : 'View profile' }} →</a>@endif
                        </div>
                        ...
                        <div class="mt-3.5 flex items-start gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if($hcFeatured->industry_fr)<p class="mt-0.5 text-[13px] md:text-[11.5px] font-semibold text-[#8A6D1F] dark:text-[#EDB33A]">{{ $isFr ? $hcFeatured->industry_fr : ($hcFeatured->industry_en ?? $hcFeatured->industry_fr) }}</p>@endif
                                @if($hcFeatured->region_fr)<p class="mt-1.5 flex items-center gap-1.5 text-[13px] md:text-[11.5px] text-[#6F6B60] dark:text-[#868778]"><i data-lucide="map-pin" class="w-3.5 h-3.5"></i> {{ $isFr ? $hcFeatured->region_fr : ($hcFeatured->region_en ?? $hcFeatured->region_fr) }}</p>@endif
                                <div class="mt-3 grid grid-cols-3 gap-2 text-center">
                                    <div><p class="text-[12px] md:text-[10px] text-[#8A857A] dark:text-[#868778]">Collections</p><p class="text-[14px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ number_format($hcFeaturedStats['collections']) }}</p></div>
                                    <div><p class="text-[12px] md:text-[10px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Produits' : 'Products' }}</p><p class="text-[14px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ number_format($hcFeaturedStats['products']) }}</p></div>
                                    <div><p class="text-[12px] md:text-[10px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Visites' : 'Visits' }}</p><p class="text-[14px] font-bold text-[#1B1B18] dark:text-[#F3EFE7]">{{ number_format($hcFeaturedStats['views']) }}</p></div>
                                </div>
                            </div>
                        </div>
                        @else
                        <p class="mt-3.5 text-[14px] md:text-[12px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Aucune donnée pour le moment.' : 'No data yet.' }}</p>
                        @endif
                        <p class="mt-3.5 bg-[#F2E8D5] dark:bg-[#3A2B06] rounded-xl px-4 py-2.5 text-center text-[13px] md:text-[11.5px] font-semibold text-[#55524A] dark:text-[#B4B5A6]">
