...
<div class="max-w-4xl">
    ...
    <div class="flex items-center gap-2 mb-5">
        <a href="{{ route('admin.moderation') }}"
            class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-colors {{ $tab === 'reports' ? 'bg-[#157A43] text-white' : 'bg-white border border-[#EFEBE2] text-[#55524A] hover:bg-[#F8F4EC]' }}">
            <i data-lucide="flag" class="w-3.5 h-3.5"></i>
            {{ $lang === 'fr' ? 'Signalements' : 'Reports' }}
            <span class="{{ $tab === 'reports' ? 'bg-white/20' : 'bg-[#F1EDE3]' }} px-1.5 rounded-full text-[10px]">{{ $reports->total() }}</span>
        </a>
        <a href="{{ route('admin.moderation', ['tab' => 'reviews']) }}"
            class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-colors {{ $tab === 'reviews' ? 'bg-[#157A43] text-white' : 'bg-white border border-[#EFEBE2] text-[#55524A] hover:bg-[#F8F4EC]' }}">
            <i data-lucide="star" class="w-3.5 h-3.5"></i>
            {{ $lang === 'fr' ? 'Avis' : 'Reviews' }}
            <span class="{{ $tab === 'reviews' ? 'bg-white/20' : 'bg-[#F1EDE3]' }} px-1.5 rounded-full text-[10px]">{{ $reviews->total() }}</span>
        </a>
    </div>
    ...
    <div class="ui-card ui-card--flush">
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        @if($report->product)
                        <a href="{{ route('products.show', $report->product->slug) }}" class="text-sm font-semibold text-[#1B1B18] hover:text-[#157A43]">
                            {{ $lang === 'fr' ? $report->product->name_fr : ($report->product->name_en ?? $report->product->name_fr) }}
                        </a>
                        @else
                        <span class="text-sm font-semibold text-[#B8B2A4]">{{ $lang === 'fr' ? 'Produit supprimé' : 'Deleted product' }}</span>
                        @endif
                    ...
                    @if($report->details)
                    <p class="text-xs text-[#8A857A] mt-1">{{ $report->details }}</p>
                    @endif
        ...
        <div class="text-center py-14 px-4">
            <i data-lucide="shield-check" class="w-10 h-10 text-[#DCD5C6] mx-auto mb-3"></i>
    ...
    <div class="ui-card ui-card--flush">
        ...
        <div class="px-5 py-3.5 border-b border-[#F5F1E8] last:border-0">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        @if($review->business)
                        <a href="{{ route('businesses.show', $review->business->slug) }}" class="text-sm font-semibold text-[#1B1B18] hover:text-[#157A43]">
                    ...
                    @if($review->title)<p class="text-xs font-medium text-[#3B382F] mt-1">{{ $review->title }}</p>@endif
                    @if($review->body)<p class="text-xs text-[#8A857A] mt-0.5">{{ Str::limit($review->body, 200) }}</p>@endif
                    <p class="text-[11px] text-[#B8B2A4] mt-1">
        ...
        @empty
        <div class="text-center py-14 px-4">
            <i data-lucide="star-off" class="w-10 h-10 text-[#DCD5C6] mx-auto mb-3"></i>
