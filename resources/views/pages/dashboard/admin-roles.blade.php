...
@section('content')
            ...
            <section class="mt-5 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                ...
                <div class="ui-card">
                    ...
                    <p class="mt-1 text-[12px] font-semibold text-[#3B382F] dark:text-[#B4B5A6]">{{ $sLabel }}</p>
                    <p class="mt-0.5 text-[11px] text-[#6F6B60] dark:text-[#868778]">{{ $sSub }}</p>
                </div>
                @endforeach
            </section>
                    ...
                    <div class="mt-4 space-y-2.5">
                        ...
                        <a href="{{ route('admin.roles', ['lang' => $lang, 'role' => $r->name]) }}" class="flex items-center gap-3 rounded-xl border px-3.5 py-3 transition-colors {{ $active ? 'border-[#14652F] dark:border-[#2E9250] bg-[#F3F9F4] dark:bg-[#12150F] relative' : 'border-[#EFEBE2] dark:border-[#262B21] hover:bg-[#FAFAF8] dark:hover:bg-[#242A1E] ' }}">
                            ...
                            <span class="min-w-0 flex-1">
                                ...
                                <span class="block text-[11px] text-[#8A857A] dark:text-[#868778]">{{ $r->user_count }} {{ $isFr ? 'utilisateurs' : 'users' }}</span>
                            </span>
                            <span class="shrink-0 rounded-md px-2 py-0.5 text-[10.5px] font-semibold {{ $r->is_system ? 'bg-[#E2F3E8] dark:bg-[#0C3D1D] text-[#157A43] dark:text-[#339B56] ' : 'bg-[#F0EAFB] dark:bg-[#1E1733] text-[#7C4FE0] dark:text-[#BCA4F0] ' }}">{{ $r->is_system ? ($isFr ? 'Système' : 'System') : ($isFr ? 'Personnalisé' : 'Custom') }}</span>
                        </a>
                        @endforeach
                    </div>
                    <p class="mt-4 text-[12px] text-[#6F6B60] dark:text-[#868778]">{{ $isFr ? 'Affichage de' : 'Showing' }} 1 {{ $isFr ? 'à' : 'to' }} {{ $roles->count() }} {{ $isFr ? 'sur' : 'of' }} {{ $roles->count() }} {{ $isFr ? 'rôles' : 'roles' }}</p>
                </section>
                ...
                <section class="ui-card">
                    <form method="POST" action="{{ route('admin.roles.update', ['id' => $selected->id]) }}">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="flex items-start gap-2.5">
                                                ...
                                                <span>
                                                    <span class="block text-[12.5px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7]">{{ $isFr ? $modFr : $modEn }}</span>
                                                </span>
                                            </div>
                                        </td>
                                        @foreach($actions as $act)
                        ...
                        <div class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#EFEBE2] dark:border-[#262B21] pt-4">
                            <div class="flex flex-wrap items-center gap-4 text-[11.5px] text-[#6F6B60] dark:text-[#868778]">
