...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mb-4 space-y-3 max-h-[55vh] overflow-y-auto">
        ...
        <div class="flex {{ $reply->is_staff ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-[80%] {{ $reply->is_staff ? 'bg-forest-600 text-white' : 'bg-gray-100 text-gray-800' }} rounded-2xl px-3.5 py-2.5">
                <p class="text-[10px] font-semibold mb-0.5 {{ $reply->is_staff ? 'text-white/70' : 'text-gray-400' }}">
                ...
                <p class="text-[10px] mt-1 {{ $reply->is_staff ? 'text-white/70' : 'text-gray-400' }}">{{ $reply->created_at->format('d/m H:i') }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <form method="POST" action="{{ route('support.reply', ['id' => $ticket->id]) }}" class="flex items-end gap-2">
        ...
        <textarea name="body" rows="2" required maxlength="3000"
            ...
            class="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-forest-400 focus:ring-1 focus:ring-forest-400 resize-none"></textarea>
    ...
    @else
    <p class="text-center text-sm text-gray-400">{{ $lang === 'fr' ? 'Ce ticket est fermé.' : 'This ticket is closed.' }}</p>
    @endif
</div>
@endsection
