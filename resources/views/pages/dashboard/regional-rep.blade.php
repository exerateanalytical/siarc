...
<div class="max-w-5xl">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            ...
            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>{{ $lang === 'fr' ? 'Entreprises' : 'Businesses' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p class="text-2xl font-bold text-gray-900">{{ $stats['products'] }}</p>
            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><i data-lucide="package" class="w-3.5 h-3.5"></i>{{ $lang === 'fr' ? 'Produits' : 'Products' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p class="text-2xl font-bold text-gray-900">{{ $stats['certified'] + $stats['verified'] }}</p>
            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><i data-lucide="badge-check" class="w-3.5 h-3.5"></i>{{ $lang === 'fr' ? 'Vérifiées/Certifiées' : 'Verified/Certified' }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['views']) }}</p>
            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><i data-lucide="eye" class="w-3.5 h-3.5"></i>{{ $lang === 'fr' ? 'Vues cumulées' : 'Total views' }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <h2 class="text-sm font-semibold text-gray-900 mb-3">{{ $lang === 'fr' ? 'Statut' : 'Status' }}</h2>
            <div class="space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-gray-500">{{ $lang === 'fr' ? 'Publiées' : 'Published' }}</span><span class="font-semibold text-green-600">{{ $stats['published'] }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">{{ $lang === 'fr' ? 'Brouillons' : 'Draft' }}</span><span class="font-semibold text-gray-500">{{ $stats['draft'] }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">{{ $lang === 'fr' ? 'Suspendues' : 'Suspended' }}</span><span class="font-semibold text-red-500">{{ $stats['suspended'] }}</span></div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">{{ $lang === 'fr' ? 'Entreprises de la région' : 'Businesses in region' }}</h2>
        </div>
        ...
        <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400">{{ $lang === 'fr' ? $business->industry->name_fr : ($business->industry->name_en ?? '') }} · {{ $business->city->name_fr ?? '' }}</p>
            </div>
            <span @class(['px-2 py-0.5 rounded-full text-xs font-medium shrink-0', 'bg-green-100 text-green-700' => $business->status === 'published', 'bg-gray-100 text-gray-500' => $business->status !== 'published'])>
                {{ ucfirst($business->status) }}
            </span>
        </div>
        @empty
        <div class="text-center py-10 text-sm text-gray-400">{{ $lang === 'fr' ? 'Aucune entreprise dans cette région.' : 'No businesses in this region.' }}</div>
        @endforelse
    </div>
        ...
        <div class="px-4 py-3 border-b border-gray-50 last:border-0">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-400">{{ $event->starts_at?->format('d/m/Y') }}</p>
                </div>
                <span class="text-xs text-gray-500 flex items-center gap-1 shrink-0"><i data-lucide="users" class="w-3.5 h-3.5"></i>{{ $event->attendees_count }}</span>
            </div>
            ...
            <div class="flex flex-wrap gap-1.5 mt-2 ml-12">
                ...
                <span class="text-xs bg-gray-50 text-gray-600 px-2 py-1 rounded-full">{{ $exhibitor->name_fr }}</span>
                @endforeach
            </div>
            @endif
        </div>
        @empty
        <div class="text-center py-8 text-sm text-gray-400">{{ $lang === 'fr' ? 'Aucune entreprise de la région n\'a participé à un événement.' : 'No businesses from this region have participated in an event.' }}</div>
        @endforelse
    </div>
