...
<div class="max-w-3xl">
    <div class="bg-white border border-[#EFEBE2] rounded-xl overflow-hidden">
        ...
        <div class="flex items-start gap-3 px-4 py-3.5 border-b border-[#FBF9F4] last:border-0">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-[#1B1B18]">{{ $meta['label'][$lang] }}</p>
                @if($log->new_values)
                <p class="text-xs text-[#A8A296]">{{ collect($log->new_values)->map(fn($v, $k) => "$k: $v")->implode(' — ') }}</p>
                @endif
            </div>
            <span class="text-xs text-[#A8A296] shrink-0">{{ $log->created_at->diffForHumans() }}</span>
        </div>
        @empty
        <div class="text-center py-12 text-sm text-[#A8A296]">
