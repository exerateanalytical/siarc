...
@section('content')
<div class="max-w-3xl">
    <div class="bg-white dark:bg-[#12150F] border border-[#EFEBE2] dark:border-[#262B21] rounded-xl overflow-hidden">
        @forelse($decisions as $log)
        @php $meta = $actionMeta[$log->action] ?? ['icon' => 'activity', 'color' => 'gray', 'label' => ['fr' => $log->action, 'en' => $log->action]]; @endphp
        <div class="flex items-start gap-3 px-4 py-3.5 border-b border-[#FBF9F4] dark:border-[#262B21] last:border-0">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-[#1B1B18] dark:text-[#F3EFE7]">{{ $meta['label'][$lang] }}</p>
                @if($log->new_values)
                <p class="text-xs text-[#A8A296] dark:text-[#868778]">{{ collect($log->new_values)->map(fn($v, $k) => "$k: $v")->implode(' — ') }}</p>
                @endif
            </div>
            <span class="text-xs text-[#A8A296] dark:text-[#868778] shrink-0">{{ $log->created_at->diffForHumans() }}</span>
        </div>
        @empty
        <div class="text-center py-12 text-sm text-[#A8A296] dark:text-[#868778]">
