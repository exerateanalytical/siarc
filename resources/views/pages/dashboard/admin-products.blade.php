...
@section('content')
                ...
                <div class="mt-2 grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-8 gap-2.5">
                    ...
                    <div class="ui-card flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[10.5px] text-[#8A857A] dark:text-[#868778] truncate">{{ $isFr ? $chip['fr'] : $chip['en'] }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="mt-5 flex flex-wrap items-center gap-1.5 border-b border-[#EAE5D8] dark:border-[#262B21]">
                ...
                <a href="{{ route('admin.products', array_filter(['lang' => $lang, 'statut' => $val, 'q' => request('q'), 'categorie' => request('categorie'), 'entreprise' => request('entreprise')], fn ($v) => $v !== null && $v !== '')) }}"
                   class="inline-flex items-center gap-1.5 px-3.5 py-2 -mb-px border-b-2 text-[12.5px] font-semibold transition-colors {{ $currentStatut === $val ? 'border-[#14652F] dark:border-[#2E9250] text-[#14652F] dark:text-[#339B56] ' : 'border-transparent text-[#6F6B60] dark:text-[#868778] hover:text-[#1B1B18] dark:hover:text-[#F3EFE7] ' }}">
                    {{ $isFr ? $tab['fr'] : $tab['en'] }}
                    <span class="rounded-full px-1.5 py-0.5 text-[10px] font-bold {{ $currentStatut === $val ? 'bg-[#E2F3E8] dark:bg-[#0C3D1D] text-[#157A43] dark:text-[#339B56] ' : 'bg-[#F3F1EA] dark:bg-[#1A1E16] text-[#6F6B60] dark:text-[#868778] ' }}">{{ number_format($tab['count']) }}</span>
                </a>
                @endforeach
            </div>
            ...
            <section class="ui-card ui-card--flush mt-4">
                            ...
                            <tr>
                                <td>
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <div class="min-w-0">
                                            <p class="text-[12.5px] font-bold text-[#1B1B18] dark:text-[#F3EFE7] truncate max-w-[220px]">{{ $isFr ? $p->name_fr : ($p->name_en ?? $p->name_fr) }}</p>
                                            <p class="text-[11px] text-[#8A857A] dark:text-[#868778] truncate max-w-[220px]">{{ $p->sku ? '#' . $p->sku : $p->slug }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <p class="text-[12px] font-medium text-[#3B382F] dark:text-[#B4B5A6]">{{ $p->business_name_fr ? ($isFr ? $p->business_name_fr : ($p->business_name_en ?? $p->business_name_fr)) : '—' }}</p>
                                    @if($p->vendor_type && isset($vendorMeta[$p->vendor_type]))
                                    <p class="text-[11px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? $vendorMeta[$p->vendor_type]['fr'] : $vendorMeta[$p->vendor_type]['en'] }}</p>
                                    @endif
                                </td>
                                ...
                                <td class="text-center">
                                    <span class="inline-block rounded-md px-2.5 py-1 text-[10.5px] font-semibold {{ $meta['pill'] ?? 'bg-[#F3F1EA] dark:bg-[#1A1E16] text-[#6F6B60] dark:text-[#868778] ' }}">{{ $meta ? ($isFr ? $meta['fr'] : $meta['en']) : $p->status }}</span>
                                </td>
                                    ...
                                    <div class="inline-flex items-center gap-1">
                                        ...
                                        <details class="row-menu relative inline-block">
                                            ...
                                            <div class="absolute right-0 z-20 mt-1 w-52 bg-white dark:bg-[#12150F] border border-[#EFEBE2] dark:border-[#262B21] rounded-xl shadow-lg py-1.5 text-left">
                                                <a href="{{ route('products.show', ['slug' => $p->slug, 'lang' => $lang]) }}" class="flex items-center gap-2 px-3.5 py-2 text-[12px] text-[#3B382F] dark:text-[#B4B5A6] hover:bg-[#F8F4EC] dark:hover:bg-[#242A1E]">
                                                    <i data-lucide="eye" class="w-3.5 h-3.5"></i>{{ $isFr ? 'Voir la fiche publique' : 'View public page' }}
                                                </a>
                                                <a href="{{ route('products.web-edit', ['slug' => $p->slug]) }}" class="flex items-center gap-2 px-3.5 py-2 text-[12px] text-[#3B382F] dark:text-[#B4B5A6] hover:bg-[#F8F4EC] dark:hover:bg-[#242A1E]">
                                                    <i data-lucide="pencil" class="w-3.5 h-3.5"></i>{{ $isFr ? 'Modifier le produit' : 'Edit product' }}
                                                </a>
                                                @if($p->business_slug)
                                                <a href="{{ route('businesses.show', ['slug' => $p->business_slug, 'lang' => $lang]) }}" class="flex items-center gap-2 px-3.5 py-2 text-[12px] text-[#3B382F] dark:text-[#B4B5A6] hover:bg-[#F8F4EC] dark:hover:bg-[#242A1E]">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 border-t border-[#F5F1E8] dark:border-[#262B21]">
                    ...
                    <nav class="flex items-center gap-1">
                        <a @if($adminProducts->onFirstPage()) aria-disabled="true" @else href="{{ $adminProducts->previousPageUrl() }}" @endif class="w-8 h-8 rounded-lg border border-[#EAE5D8] dark:border-[#262B21] flex items-center justify-center {{ $adminProducts->onFirstPage() ? 'text-[#C9C3B4] dark:text-[#868778] cursor-default' : 'text-[#3B382F] dark:text-[#B4B5A6] hover:border-[#14652F] dark:hover:border-[#2E9250] hover:text-[#14652F] dark:hover:text-[#339B56] ' }}">
                            <i data-lucide="chevron-left" class="w-4 h-4"></i>
                        </a>
                        @if($from > 1)
                        <a href="{{ $adminProducts->url(1) }}" class="w-8 h-8 rounded-lg border border-[#EAE5D8] dark:border-[#262B21] flex items-center justify-center text-[12px] font-semibold text-[#3B382F] dark:text-[#B4B5A6] hover:border-[#14652F] dark:hover:border-[#2E9250] hover:text-[#14652F] dark:hover:text-[#339B56]">1</a>
                        @if($from > 2)<span class="px-1 text-[12px] text-[#8A857A] dark:text-[#868778]">…</span>@endif
                        @endif
                        @foreach($adminProducts->getUrlRange($from, $to) as $page => $url)
                        <a href="{{ $url }}" class="w-8 h-8 rounded-lg flex items-center justify-center text-[12px] font-semibold {{ $page === $cur ? 'bg-[#0F4824] dark:bg-[#2E9250] text-white dark:text-[#04150A]' : 'border border-[#EAE5D8] dark:border-[#262B21] text-[#3B382F] dark:text-[#B4B5A6] hover:border-[#14652F] dark:hover:border-[#2E9250] hover:text-[#14652F] dark:hover:text-[#339B56] ' }}">{{ $page }}</a>
                        @endforeach
                        @if($to < $last)
                        @if($to < $last - 1)<span class="px-1 text-[12px] text-[#8A857A] dark:text-[#868778]">…</span>@endif
                        <a href="{{ $adminProducts->url($last) }}" class="w-8 h-8 rounded-lg border border-[#EAE5D8] dark:border-[#262B21] flex items-center justify-center text-[12px] font-semibold text-[#3B382F] dark:text-[#B4B5A6] hover:border-[#14652F] dark:hover:border-[#2E9250] hover:text-[#14652F] dark:hover:text-[#339B56]">{{ number_format($last) }}</a>
                        @endif
                        <a @if(!$adminProducts->hasMorePages()) aria-disabled="true" @else href="{{ $adminProducts->nextPageUrl() }}" @endif class="w-8 h-8 rounded-lg border border-[#EAE5D8] dark:border-[#262B21] flex items-center justify-center {{ $adminProducts->hasMorePages() ? 'text-[#3B382F] dark:text-[#B4B5A6] hover:border-[#14652F] dark:hover:border-[#2E9250] hover:text-[#14652F] dark:hover:text-[#339B56] ' : 'text-[#C9C3B4] dark:text-[#868778] cursor-default' }}">
                            <i data-lucide="chevron-right" class="w-4 h-4"></i>
                        </a>
                    </nav>
                    @endif
                    <span class="text-[12px] text-[#6F6B60] dark:text-[#868778] bg-[#F8F4EC] dark:bg-[#1A1E16] border border-[#EFEBE2] dark:border-[#262B21] rounded-lg px-3 py-1.5">10 / page</span>
                </div>
            </section>
            ...
            <div class="mt-5 grid lg:grid-cols-2 gap-4">
                ...
                <section class="ui-card">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.industries', ['lang' => $lang]) }}" class="text-[11.5px] font-semibold text-[#14652F] dark:text-[#339B56] hover:underline">{{ $isFr ? 'Voir tout →' : 'View all →' }}</a>
                    </div>
                    @php
                    ...
                    <p class="mt-4 text-[12.5px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Aucun produit.' : 'No products.' }}</p>
                    @else
                    <div class="mt-4 flex flex-col sm:flex-row items-center gap-5">
                        <div class="relative w-[132px] h-[132px] shrink-0 rounded-full" style="background: conic-gradient({{ implode(', ', $stops) }});">
                            <div class="absolute inset-[18px] bg-white dark:bg-[#12150F] rounded-full flex flex-col items-center justify-center">
                                ...
                                <p class="text-[10px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Produits' : 'Products' }}</p>
                            </div>
                        </div>
                        <ul class="flex-1 w-full space-y-2">
                            ...
                            <li class="flex items-center gap-2.5">
                                ...
                                <span class="flex-1 min-w-0 text-[12px] text-[#3B382F] dark:text-[#B4B5A6] truncate">{{ $c->name_fr ? ($isFr ? $c->name_fr : ($c->name_en ?? $c->name_fr)) : ($isFr ? 'Sans catégorie' : 'Uncategorized') }}</span>
                                <span class="text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7] whitespace-nowrap">{{ number_format($c->cnt) }} <span class="font-normal text-[#8A857A] dark:text-[#868778]">({{ $c->pct }}%)</span></span>
                            </li>
                            @endforeach
                            @if($othersCnt > 0)
                            <li class="flex items-center gap-2.5">
                                <span class="w-2.5 h-2.5 rounded-sm shrink-0 bg-[#D6D3D1] dark:bg-[#1A1E16]"></span>
                                <span class="flex-1 min-w-0 text-[12px] text-[#3B382F] dark:text-[#B4B5A6] truncate">{{ $isFr ? 'Autres catégories' : 'Other categories' }}</span>
                                <span class="text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7] whitespace-nowrap">{{ number_format($othersCnt) }} <span class="font-normal text-[#8A857A] dark:text-[#868778]">({{ $othersPct }}%)</span></span>
                            </li>
                            @endif
                        </ul>
                    </div>
                ...
                <section class="ui-card">
                    ...
                    @if($pricedTotal === 0)
                    <p class="mt-4 text-[12.5px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Aucun produit avec un prix renseigné.' : 'No products with a price set.' }}</p>
                    @else
                    <p class="mt-0.5 text-[11.5px] text-[#8A857A] dark:text-[#868778]">{{ $isFr ? 'Répartition des ' . number_format($pricedTotal) . ' produits avec prix renseigné.' : 'Distribution of the ' . number_format($pricedTotal) . ' products with a price set.' }}</p>
                    <ul class="mt-4 space-y-3.5">
                        @foreach($priceRanges as $range)
                        <li>
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-[12px] text-[#3B382F] dark:text-[#B4B5A6]">{{ $isFr ? $range['fr'] : $range['en'] }}</span>
                                <span class="text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7] whitespace-nowrap">{{ number_format($range['cnt']) }} <span class="font-normal text-[#8A857A] dark:text-[#868778]">({{ $range['pct'] }}%)</span></span>
                            </div>
