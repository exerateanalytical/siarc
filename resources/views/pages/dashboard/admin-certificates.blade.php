...
<div class="max-w-[1400px]">
    ...
    <div class="mb-4 flex items-center gap-2 bg-[#E9F6EE] dark:bg-[#0C3D1D] border border-[#BFE3CD] dark:border-[#39402F] rounded-xl px-4 py-2.5 text-[14px] md:text-[12.5px] text-[#0F5B30] dark:text-[#339B56]"><i data-lucide="check-circle-2" class="w-4 h-4"></i>{{ $isFr ? 'Certificat mis à jour.' : 'Certificate updated.' }}</div>
    @endif
    ...
    <div class="ui-card ui-card--flush">
            ...
            <table class="ui-table">
                ...
                <tbody>
                    ...
                    <tr class="border-t border-[#EFEBE2] dark:border-[#262B21] hover:bg-[#FAFAF8] dark:hover:bg-[#242A1E]">
                        <td><span class="text-[14px] md:text-[12px] font-semibold text-[#1B1B18] dark:text-[#F3EFE7] font-mono">{{ $r->certificate_no }}</span></td>
                        ...
                        <td><span class="text-[12px] md:text-[11px] font-semibold px-2 py-0.5 rounded-full" style="background-color: {{ $t[0] }};color: {{ $t[1] }}">{{ $t[2] }}</span></td>
                        <td class="text-right">
        ...
        <div class="px-5 py-3.5 border-t border-[#EFEBE2] dark:border-[#262B21]">
            <p class="text-[14px] md:text-[12px] text-[#8A857A] dark:text-[#868778]">{{ number_format($rows->count()) }} {{ $isFr ? 'certificat(s) affiché(s)' : 'certificate(s) shown' }}</p>
        </div>
    </div>
</div>
@endsection
