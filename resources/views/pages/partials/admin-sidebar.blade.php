...
<aside id="ad-sidebar" class="shrink-0 lg:w-[290px] bg-gradient-to-b from-[#042B15] to-[#03200E] dark:from-[#0C3B1E] dark:to-[#093017] dark:border-r dark:border-[#2A6B41] text-white flex flex-col relative">
    <div class="px-4 pt-4 pb-3 flex items-center gap-3">
        ...
        <div class="leading-tight min-w-0">
            <p class="text-[14px] md:text-[11px] font-bold tracking-[0.02em] text-white uppercase leading-snug">{{ $isFr ? 'Artisan Hub 237' : 'Artisan Hub 237' }}</p>
            <p class="mt-1 text-[14px] md:text-[9.5px] italic text-[#E9C25A] leading-snug">{{ $isFr ? 'Notre Héritage, Notre Fierté, Notre Avenir' : 'Our Heritage, Our Pride, Our Future' }}</p>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-4 pb-4 pt-1">
        @foreach($adGroups as [$adTitle, $adItems])
            @if($adTitle)
            <p class="mt-5 mb-2 px-1 text-[14px] md:text-[11.5px] font-bold tracking-[0.02em] text-[#E6B201] uppercase truncate">{{ $adTitle }}</p>
            @endif
            @foreach($adItems as [$adKey, $adIcon, $adLabel, $adUrl, $adBadge])
                ...
                <a href="{{ $adUrl }}" aria-current="page" class="mt-0.5 flex items-center gap-3 bg-[#0D5A30] dark:bg-[#2E9250] rounded-xl px-3 py-[9px] min-h-[44px] text-[16px] md:text-[13px] font-bold text-white dark:text-[#04150A] shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] dark:shadow-none">
                    <i data-lucide="{{ $adIcon }}" class="w-[17px] h-[17px] shrink-0" style="stroke-width:1.9"></i>
                    <span class="flex-1 truncate">{{ $adLabel }}</span>
                    @if($adBadge)<span class="shrink-0 bg-[#DE8E14] text-white dark:text-[#2A1902] text-[14px] md:text-[10.5px] font-bold rounded-md px-2 py-0.5">{{ $adBadge }}</span>@endif
                </a>
                @else
                <a href="{{ $adUrl }}" class="flex items-center gap-3 rounded-xl px-3 py-[7px] min-h-[44px] text-[16px] md:text-[12.5px] {{ $adKey === 'dashboard' ? 'font-bold tracking-[0.03em] text-white' : 'text-[#EDF2EC]' }} hover:bg-white/5 transition-colors">
    ...
    <div class="shrink-0 relative">
        ...
        <p class="absolute left-[9%] top-[15%] w-[62%] font-serif text-[14px] leading-[1.6] md:text-[13.5px] text-[#F0EBDD]">{{ $adSideQuote }}</p>
    </div>
</aside>
