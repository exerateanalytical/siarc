    ...
    <div class="p-2">
        ...
        <details class="group border-b border-[#F4F3EF] dark:border-[#262B21] last:border-0">
            <summary class="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-[#F6F7F6] hover:dark:bg-[#0A0C09] cursor-pointer list-none marker:hidden">
                ...
                <span class="text-[12.5px] font-semibold text-[#1D1B16] dark:text-[#F3EFE7] flex-1 truncate">{{ $sbFr ? $sec->name_fr : ($sec->name_en ?? $sec->name_fr) }}</span>
                <span class="text-[10.5px] text-[#8A857A] dark:text-[#868778] shrink-0">{{ $sec->filieres->count() }}</span>
            </summary>
            <div class="pl-[26px] pr-1 pb-1.5">
                <a href="{{ route('industries.index', ['lang' => $sbLang, 'cat' => $sec->slug]) }}" class="block px-2 py-1.5 text-[11.5px] font-semibold text-[#157A43] dark:text-[#339B56] hover:underline">{{ $sbFr ? 'Tout le secteur →' : 'Whole sector →' }}</a>
                @foreach($sec->filieres as $fil)
                @php $sbFilName = $sbFr ? $fil->name_fr : ($fil->name_en ?? $fil->name_fr); @endphp
                <a href="{{ route('industries.index', ['lang' => $sbLang, 'cat' => $fil->slug]) }}" title="{{ $sbFilName }}" class="block px-2 py-1.5 text-[12px] text-[#3A3A35] dark:text-[#F3EFE7] hover:text-leaf hover:dark:text-[#339B56] hover:bg-[#FAF8F2] hover:dark:bg-[#0A0C09] rounded truncate">{{ $sbFilName }}</a>
                @endforeach
            </div>
        </details>
        @endforeach
        <a href="{{ route('industries.index', ['lang' => $sbLang]) }}" class="mt-1 flex items-center gap-1.5 px-2 py-2 text-[12px] font-semibold text-[#157A43] dark:text-[#339B56] hover:underline">
