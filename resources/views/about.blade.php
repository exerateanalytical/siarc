...
<!-- Tricolor top bar with star -->
<div class="relative flex h-2.5">
    <div class="flex-1 bg-flagg"></div>
    <div class="flex-1 bg-flagr"></div>
    <div class="flex-1 bg-flagy"></div>
    <svg viewBox="0 0 24 24" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 fill-flagy" aria-hidden="true">
...
<header class="bg-cream">
    <div class="max-w-[1280px] mx-auto px-5 lg:px-8">
        <div class="flex items-center justify-between gap-4 py-3.5">
            ...
            <nav class="hidden lg:flex items-center gap-3.5 xl:gap-[22px]">
                @foreach($navLinks as [$navLabel, $navHref, $navActive])
                <a href="{{ $navHref }}" class="relative whitespace-nowrap text-[12px] xl:text-[13px] font-medium {{ $navActive ? 'text-[#1D1B16]' : 'text-[#262521]' }} hover:text-leaf transition-colors pb-1">
                    {{ $navLabel }}
                    @if($navActive)<span class="absolute left-0 right-0 -bottom-0.5 h-[2px] bg-goldlt"></span>@endif
                </a>
                @endforeach
            </nav>

            <div class="flex items-center gap-3 shrink-0">
                <!-- Search (icon only, expands to the search page) -->
                <a href="{{ route('gallery.search', ['lang' => $lang]) }}" aria-label="{{ $isFr ? 'Rechercher' : 'Search' }}"
                    class="hidden sm:flex w-9 h-9 items-center justify-center rounded-md text-[#262521] hover:bg-sand/50 transition-colors">
        ...
        <div id="mobile-menu" class="hidden lg:hidden pb-4 border-t border-sand pt-3">
            @foreach($navLinks as [$navLabel, $navHref, $navActive])
            <a href="{{ $navHref }}" class="block px-2 py-2 text-[14px] font-medium {{ $navActive ? 'text-leaf' : 'text-[#262521]' }} hover:bg-sand/40 rounded">{{ $navLabel }}</a>
            @endforeach
...
<footer class="bg-pine">
    <div class="max-w-[1280px] mx-auto px-5 lg:px-8 pt-9 pb-5">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.35fr_0.7fr_0.95fr_0.85fr_1.15fr] gap-7">
            <!-- Brand -->
            <div>
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images/landing/about-footer-logo.png') }}" alt="" class="w-10 h-10 rounded-full">
                ...
                <p class="mt-4 text-[12px] text-sage leading-relaxed max-w-[280px]">
                    {{ $isFr
                        ? 'Notre héritage, notre fierté, notre avenir. Plateforme officielle de promotion et de valorisation de l\'artisanat camerounais.'
                        : 'Our heritage, our pride, our future. Official platform for the promotion of Cameroonian craftsmanship.'
                    }}
                </p>
            ...
            <div>
                ...
                <form action="/inscription" method="GET" class="mt-4 flex gap-2">
                    ...
                    <input name="email" type="email" required placeholder="{{ $isFr ? 'Votre email' : 'Your email' }}"
                        class="flex-1 min-w-0 bg-transparent border border-white/25 rounded-md px-3 py-2.5 text-[12px] text-white placeholder-sage/70 focus:outline-none focus:border-gold">
        ...
        <!-- Legal bar -->
        <div class="mt-8 pt-4 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-[11.5px] text-sage/80">
