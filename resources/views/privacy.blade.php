...
<div class="max-w-2xl mx-auto px-4 py-8">
    <h1 class="text-xl font-bold text-[#1B1B18] mb-1">{{ $lang === 'fr' ? 'Politique de confidentialité' : 'Privacy Policy' }}</h1>
    ...
    <div class="space-y-6 text-sm text-[#6F6B60] leading-relaxed">
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">1. {{ $lang === 'fr' ? 'Données que nous collectons' : 'Data we collect' }}</h2>
            <div class="overflow-x-auto">
                <table class="w-full text-xs border-collapse">
                    <tr class="border-b border-[#EFEBE2] font-semibold text-[#3B382F]"><td class="py-1.5 pr-3">{{ $lang === 'fr' ? 'Catégorie' : 'Category' }}</td><td class="py-1.5">{{ $lang === 'fr' ? 'Exemples' : 'Examples' }}</td></tr>
                    <tr class="border-b border-[#F1EDE4]"><td class="py-1.5 pr-3">{{ $lang === 'fr' ? 'Compte' : 'Account' }}</td><td class="py-1.5">{{ $lang === 'fr' ? 'Nom, email, téléphone, mot de passe' : 'Name, email, phone, password' }}</td></tr>
                    <tr class="border-b border-[#F1EDE4]"><td class="py-1.5 pr-3">{{ $lang === 'fr' ? 'Entreprise' : 'Business' }}</td><td class="py-1.5">{{ $lang === 'fr' ? 'Nom commercial, adresse, logo, produits' : 'Business name, address, logo, products' }}</td></tr>
                    <tr class="border-b border-[#F1EDE4]"><td class="py-1.5 pr-3">{{ $lang === 'fr' ? 'Messages' : 'Messages' }}</td><td class="py-1.5">{{ $lang === 'fr' ? 'Contenu des échanges entre acheteurs et vendeurs' : 'Content of exchanges between buyers and sellers' }}</td></tr>
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">2. {{ $lang === 'fr' ? 'Utilisation des données' : 'How we use your data' }}</h2>
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">3. {{ $lang === 'fr' ? 'Partage des données' : 'Data sharing' }}</h2>
            <p>{{ $lang === 'fr'
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">4. {{ $lang === 'fr' ? 'Conservation des données' : 'Data retention' }}</h2>
            <p>{{ $lang === 'fr'
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">5. {{ $lang === 'fr' ? 'Vos droits' : 'Your rights' }}</h2>
            <p>{{ $lang === 'fr'
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">6. {{ $lang === 'fr' ? 'Sécurité' : 'Security' }}</h2>
            <p>{{ $lang === 'fr'
        ...
        <section>
            <h2 class="text-sm font-semibold text-[#1B1B18] mb-1.5">7. {{ $lang === 'fr' ? 'Cookies' : 'Cookies' }}</h2>
            <p>{{ $lang === 'fr'
