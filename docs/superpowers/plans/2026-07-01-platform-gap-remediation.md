...
- [x] **Step 1:** Delete legacy blocks section by section (work bottom-up so line numbers stay valid). After each section removal run: `php artisan route:list > /dev/null && echo OK`
- [x] **Step 2:** Grep for stragglers: `grep -nE "job_postings|user_cvs|collabcam|communities|federations|kyc_applications|wallets|tenders|share_offerings|salary_reports|digital_cards|associations|supplier_reviews|logistics_listings|innovation_projects|knowledge_resources|shared_assets|employee_profiles|invest_|cover_letters|company_users" routes/web.php` — expected: no matches.
- [x] **Step 3:** Confirm the duplicate `admin.users` name is gone: `php artisan route:list 2>/dev/null | grep -c "admin.users "` → exactly 1.
- [x] **Step 4:** Run the route-reference audit (used routes vs defined routes diff) — expected: empty diff. Any view still referencing a deleted route name gets handled in Task 2.
- [x] **Step 5:** Commit: `git commit -m "Remove dead legacy app routes (jobs/CV/collabcam/invest/old admin)"`
...
- [x] **Step 1:** Build the deletion list mechanically: for every blade file, check whether any surviving route or view references it (`view('...')` in routes/controllers + `@include`/`@extends` in blades). Delete only unreferenced ones tied to legacy features.
- [x] **Step 2:** Re-run the route audit; fix any surviving view that links to a deleted route (e.g., old navbars in `layouts/app.blade.php` linking `/jobs`).
- [x] **Step 3:** Browser smoke: load `/`, `/login`, `/galerie/entreprises`, `/evenements`, `/partenaires` — 200s, no missing-view exceptions.
- [x] **Step 4:** Commit: `git commit -m "Remove legacy views orphaned by route purge"`
...
- [x] **Step 1:** `grep -n "auth_user" routes/web.php app resources -r` — for each survivor, port to `siac_user` (same keys: id, name, email, role, is_admin) or delete if it was legacy-only. *(Done 2026-07-02: also deleted the legacy `/welcome` onboarding flow + view, rewrote legacy `POST /register` to create SIAC users, and removed orphaned `routes/web_phase2.php`.)*
- [x] **Step 2:** Port `/developer` portal (uses existing `api_consumers`/`api_keys` tables) to `siac_user` auth; keep functionality (create/revoke API keys). *(Rewired to the real schema: consumer matched by email, keys under `consumer_id`; old code referenced nonexistent `api_keys.user_id`.)*
- [x] **Step 3:** Verify login as each demo account still works; verify `/developer` works logged in as entrepreneur; verify grep for `auth_user` returns zero hits.
- [x] **Step 4:** Commit: `git commit -m "Unify web auth on siac_user session; port developer portal"`
